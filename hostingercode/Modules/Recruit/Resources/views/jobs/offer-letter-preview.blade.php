...
<head>
    ...
    <style>
        @php
            // Use $company from controller instead of company() helper for front-end context
            $companyId = isset($company) ? $company->id : (function_exists('company') ? company()->id : ($jobOffer->company_id ?? null));
            $setting = \Modules\Recruit\Entities\RecruitSetting::where('company_id', $companyId)->first();
            if ($setting && $setting->offer_letter_background_image) {
                $bgImageUrl = asset_url_local_s3('offer-letter-background/' . $setting->offer_letter_background_image);
        @endphp
        body {
            background-image: url(@php echo $bgImageUrl; @endphp);
        ...
        .offer-letter-content h1,
        .offer-letter-content h2,
        .offer-letter-content h3,
        .offer-letter-content h4,
        .offer-letter-content h5,
        .offer-letter-content h6 {
            margin-top: 20px;
            margin-bottom: 12px;
            font-weight: bold;
        }

        .offer-letter-content strong {
            font-weight: bold;
        }

        .offer-letter-content em {
            font-style: italic;
        }

        .offer-letter-content ul,
        .offer-letter-content ol {
            margin-left: 20px;
            margin-bottom: 12px;
        }

        .offer-letter-content li {
            margin-bottom: 6px;
        }
...
<body id="body" class="h-100 bg-additional-grey" @php
    // Use $company from controller instead of company() helper for front-end context
    $companyId = isset($company) ? $company->id : (function_exists('company') ? company()->id : ($jobOffer->company_id ?? null));
    $setting = \Modules\Recruit\Entities\RecruitSetting::where('company_id', $companyId)->first();
    if ($setting && $setting->offer_letter_background_image) {
        $bgImageUrl = asset_url_local_s3('offer-letter-background/' . $setting->offer_letter_background_image);
        echo 'style="background-image: url(' . $bgImageUrl . '); background-size: cover; background-position: center; background-repeat: no-repeat; background-attachment: fixed;"';
    }
@endphp>
    ...
    <div class="body-wrapper clearfix">
        ...
        <section class="bg-additional-grey" id="fullscreen" @php
            if ($setting && $setting->offer_letter_background_image) {
                echo 'style="background-color: rgba(255, 255, 255, 0);"';
            }
        @endphp>
            ...
            <div class="content-wrapper container">
                ...
                @php
                    // Use $company from controller instead of company() helper for front-end context
                    $companyId = isset($company) ? $company->id : (function_exists('company') ? company()->id : ($jobOffer->company_id ?? null));
                    $bgSetting = \Modules\Recruit\Entities\RecruitSetting::where('company_id', $companyId)->first();
                @endphp
                <div class="card border-0 invoice" @if($bgSetting && $bgSetting->offer_letter_background_image) style="background-color: rgba(255, 255, 255, 0.95);" @endif>
                    <!-- CARD BODY START -->
                    <div class="card-body">
                        <div class="invoice-table-wrapper">
                            ...
                            @php
                                // Use $company from controller - it's set in jobOfferLetter() method as $this->company
                                // In Blade views, controller properties are accessible directly
                                $companyId = isset($company) && $company ? $company->id : $jobOffer->company_id;
                                $setting = \Modules\Recruit\Entities\RecruitSetting::where('company_id', $companyId)->first();
                                $offerContent = ($setting && $setting->offer_letter_content) ? $setting->offer_letter_content : '';
                                
                                if ($offerContent) {
                                    $companyForCurrency = isset($company) ? $company : (function_exists('company') ? company() : null);
                                    $currencyId = ($currency ? $currency->id : ($companyForCurrency && $companyForCurrency->currency ? $companyForCurrency->currency->id : 1));
                                    $offerContent = \Modules\Recruit\Services\OfferLetterVariableService::replace(
                                        $offerContent,
                                        $jobOffer,
                                        $companyForCurrency,
                                        $currencyId,
                            ...
                            @if($offerContent)
                                <!-- Custom Offer Letter Template Content -->
                                <div class="row mt-4 mb-4">
                                    <div class="col-sm-12">
                                        <div class="card-body offer-letter-content">
                                            {!! $offerContent !!}
                                        </div>
                                    </div>
                                </div>
                            @else
                                <!-- Fallback: Show default sections if template not set -->
                                <div class="row">
                                    <div class="col-sm-12">
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="col-12 px-0 pb-3 d-block d-lg-flex d-md-flex">
                                            ...
                                            <p class="mb-0 text-dark-grey f-14 w-70">
                                                {{ $jobOffer->jobApplication->location->location ?? '' }}
                                            </p>
                                        </div>
                                    </div>
                                </div>
