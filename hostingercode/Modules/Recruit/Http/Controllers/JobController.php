...
class JobController extends AccountBaseController
{
    ...
    {
        try {
            $this->job = RecruitJob::with('team', 'files', 'employee', 'jobType', 'category', 'subcategory')->withTrashed()->findOrFail($id);
            
            $this->pageTitle = ucwords($this->job->title ?? __('recruit::app.menu.job'));
            ...
            $this->openingsCount = $this->job->total_positions ?? 0;
            ...
            if (request()->ajax()) {
                return Reply::error(__('messages.jobNotFound'));
            }
            abort(404, __('messages.jobNotFound'));
        } catch (\Exception $e) {
            ...
            abort(500, __('messages.somethingWentWrong'));
        }
    }
    ...
    {
        try {
            // Get jobId from request - check multiple possible parameter names
            $jobId = $request->jobId ?? $request->id ?? $request->job_id;
            ...
            if (!$status) {
                return Reply::error(__('messages.statusRequired'));
            }
            
            // Validate status value
            if (!in_array($status, ['open', 'closed'])) {
                return Reply::error(__('messages.invalidStatus'));
            }
