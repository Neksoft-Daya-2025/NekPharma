...
class FrontJobController extends FrontBaseController
{
    ...
    public function jobApply($slug, $locationId, $company)
    {
        try {
            ...
            // Ensure setting exists with default values if null
            if (!$this->setting) {
                $this->setting = new RecruitSetting();
                $this->setting->company_id = $this->companyData->id;
                $this->setting->type = 'bg-color';
            }
            ...
            if (!$this->job) {
                abort(404, __('recruit::messages.jobNotFound'));
            }
            ...
            // Ensure recruitSetting exists with default values if null to prevent view errors
            if (!$this->recruitSetting) {
                $this->recruitSetting = new RecruitSetting();
                $this->recruitSetting->company_id = $this->companyData->id;
                $this->recruitSetting->background_color = '#ffffff';
                $this->recruitSetting->type = 'bg-color';
                $this->recruitSetting->legal_term = null;
                $this->recruitSetting->google_recaptcha_status = 'inactive';
                $this->recruitSetting->background_image = null;
                $this->recruitSetting->logo = null;
                // Set company relationship for accessor methods
                $this->recruitSetting->setRelation('company', $this->companyData);
            }
            
            try {
                $this->applicationSources = ApplicationSource::all();
            } catch (\Exception $e) {
                \Log::warning('Error loading application sources: ' . $e->getMessage());
                $this->applicationSources = collect([]);
            }
            ...
            try {
                ...
                \Log::warning('Error fetching questions: ' . $e->getMessage());
                $this->fields = [];
            }

            $this->allQuestions = $this->job ? ($this->job->question ?? collect([])) : collect([]);

            return view('recruit::front.job-apply', $this->data);
        } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $e) {
            \Log::error('Model not found in jobApply: ' . $e->getMessage(), [
                'exception' => $e,
                'slug' => $slug,
            ...
            \Log::error('Error loading job apply page: ' . $e->getMessage(), [
                'exception' => $e,
                'slug' => $slug,
                'locationId' => $locationId,
                'company' => $company,
            ...
            abort(500, __('messages.somethingWentWrong') . ': ' . $e->getMessage());
        }
    }
    ...
    public function saveApplication(FrontJobApplication $request)
    {
        ...
        if($recruitSetting && $recruitSetting->google_recaptcha_status == 'active'){
        ...
        if (!$status) {
            return Reply::error(__('recruit::messages.applicationStatusNotFound'));
        }
        ...
        // Get the job for notification
        $job = RecruitJob::find($request->job_id);
        ...
        if (!empty($request->skill_id)) {
            // Use the correct application ID (the one we just created)
            RecruitApplicationSkill::where('recruit_job_application_id', $jobApplication->id)->delete();
        ...
        if (!empty($request->answer)) {
            ...
            foreach ($request->answer as $key => $value) {
                ...
                if($request->hasFile('answer.' . $key)){
                    ...
                    \Log::warning('Custom question not found: ' . $key, ['exception' => $e]);
                    // Continue with next answer
                    continue;
                } catch (\Exception $e) {
                    \Log::error('Error saving custom answer: ' . $e->getMessage(), [
                        'exception' => $e,
                        'question_id' => $key
                    ]);
                    // Continue with next answer
        ...
        try {
            $users = User::allAdmins($jobApplication->company_id);
            ...
            \Log::error('Model not found in saveApplication: ' . $e->getMessage(), [
                'exception' => $e,
                'request' => $request->all()
            ]);
    ...
    public function fetchQuestion($SelectedQuestions)
    {
        ...
        if($SelectedQuestions != null && is_iterable($SelectedQuestions)){
            foreach ($SelectedQuestions as $group) {
                try {
                    if (!is_object($group) || !isset($group->recruit_custom_question_id)) {
                        \Log::warning('Invalid group object in fetchQuestion', ['group' => $group]);
                        continue;
                    }
                    ...
                    \Log::warning('Error processing question group: ' . $e->getMessage(), [
                        'exception' => $e,
                        'group' => $group
                    ]);
                    continue;
