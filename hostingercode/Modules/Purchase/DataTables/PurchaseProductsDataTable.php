...
class PurchaseProductsDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('action', function ($row) {
            ...
            // Duplicate button as icon (outside dropdown)
            if ($this->addProductPermission == 'all' || $this->addProductPermission == 'added') {
                $action .= '<a class="btn btn-secondary btn-sm rounded mr-1 openRightModal" href="' . route('purchase-products.create') . '?duplicate_product=' . $row->id . '" data-toggle="tooltip" data-original-title="' . trans('app.duplicate') . '">
            ...
            $action .= '<a href="' . route('purchase-products.show', [$row->id]) . '" class="dropdown-item productView" data-product-id="' . $row->id . '"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';
            ...
            // Stock Adjustments option
            $editInventoryPermission = user()->permission('edit_product');
            if (($row->type == 'goods') && ($editInventoryPermission == 'all' || $editInventoryPermission == 'added')) {
                $action .= '<a class="dropdown-item stock-adjustment-btn" href="javascript:;" data-product-id="' . $row->id . '">
                                <i class="fa fa-sliders mr-2"></i>
                                ' . trans('purchase::app.adjustStock') . '
                            </a>';
            }
        ...
        $datatables->editColumn('stock_on_hand', function ($row) {
            ...
            foreach ($row->inventory as $inventory) {
                return $inventory->net_quantity ?: 0;
            }

        });

        $datatables->editColumn('stock', function ($row) {
            // Get stock from PurchaseStockAdjustment.net_quantity (sum in case multiple rows exist per product)
            // This reflects: Purchase Entries (increase) - CFA Distributor Invoices (decrease)
            $companyId = company()->id;

            $adjustmentRows = PurchaseStockAdjustment::where('product_id', $row->id)
                ->where('company_id', $companyId)
                ->get(['net_quantity']);

            if ($adjustmentRows->isNotEmpty()) {
                return number_format((float) $adjustmentRows->sum('net_quantity'), 0);
            }

            // Fallback: Calculate from purchase entries if no PurchaseStockAdjustment rows exist
            // This should only happen for products that haven't had any stock adjustments yet
            $entries = \App\Models\ProductPurchaseDetail::where('product_id', $row->id)
            ...
            return number_format($totalPurchaseEntries, 0);
        });
        ...
        $datatables->editColumn('packing', function ($row) {
            return $row->packing ?: '--';
        });

        $datatables->editColumn('hsn_sac_code', function ($row) {
            return $row->hsn_sac_code ?: '--';
        });

        $datatables->editColumn('total', function ($row) {
            return $row->total ? currency_format($row->total) : '--';
        });
        ...
        $datatables->rawColumns(array_merge(['action', 'price', 'allow_purchase', 'check', 'name', 'default_image', 'status', 'total'], $customFieldColumns));
        ...
        if ($request->searchText != '') {
            $model->where(function ($query) {
                $query->where('products.name', 'like', '%' . request('searchText') . '%')
                    ->orWhere('products.packing', 'like', '%' . request('searchText') . '%')
                    ->orWhere('products.price', 'like', '%' . request('searchText') . '%')
                    ->orWhere('products.total', 'like', '%' . request('searchText') . '%')
                    ->orWhere('products.hsn_sac_code', 'like', '%' . request('searchText') . '%');
            });
        }
    ...
    protected function getColumns()
    {
        ...
        $data = [
            ...
            __('Packing') => ['data' => 'packing', 'name' => 'packing', 'title' => __('Packing')],
            Column::make('hsn_sac_code')->title('HSN')->name('hsn_sac_code'),
            __('Stock') => ['data' => 'stock', 'name' => 'stock', 'title' => __('Stock')],
