        ...
        <div class="d-flex justify-content-between action-bar">
            <div id="table-actions" class="flex-grow-1 align-items-center">
                @if ($addProductPermission == 'all' || $addProductPermission == 'added')
                    <x-forms.link-primary :link="route('purchase-products.create')" class="mr-3 openRightModal"
                        icon="plus">
                        @lang('app.add')
                        @lang('app.product')
                    </x-forms.link-primary>
                    <x-forms.link-secondary :link="route('purchase-products.import')" class="mr-3 openRightModal"
                        icon="upload">
                        @lang('app.importExcel')
                    </x-forms.link-secondary>
                @endif
                @if (user()->permission('edit_product') == 'all')
                    <button type="button" class="btn btn-warning mr-3" id="recalculate-stock-btn" data-toggle="tooltip" data-original-title="@lang('purchase::app.recalculateStockTooltip')">
                        <i class="fa fa-calculator mr-1"></i>
                        @lang('purchase::app.recalculateStock')
                    </button>
                @endif
            </div>
            <div id="emptyCartBox">
...
@push('scripts')
    ...
    <script>
        ...
        $('body').on('click', '.stock-adjustment-btn', function() {
        ...
        // Recalculate Stock Button
        $('#recalculate-stock-btn').click(function() {
            Swal.fire({
                title: "@lang('messages.sweetAlertTitle')",
                text: "@lang('purchase::app.recalculateStockConfirm')",
                icon: 'warning',
                showCancelButton: true,
                focusConfirm: false,
                confirmButtonText: "@lang('messages.confirmDelete')",
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('purchase_products.recalculate_stock') }}";
                        ...
                        success: function(response) {
                            if (response.status == "success") {
                                Swal.fire({
                                    ...
                                    // Reload the table to show updated stock
                                    showTable();
                                });
                            }
                        },
