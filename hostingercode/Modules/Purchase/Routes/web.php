...
Route::group(['middleware' => 'auth', 'prefix' => 'account'], function () {
    ...
    Route::post('purchase-products/recalculate-stock', [PurchaseProductController::class, 'recalculateStock'])->name('purchase_products.recalculate_stock');
    ...
    /* Product Import */
    Route::group(
        ['prefix' => 'purchase-products'],
        function () {
            Route::get('import', [PurchaseProductController::class, 'importProduct'])->name('purchase-products.import');
            Route::get('import/sample', [PurchaseProductController::class, 'downloadSample'])->name('purchase-products.import.sample');
    ...
    Route::get('purchase-order/get-products', [PurchaseOrderController::class, 'getProducts'])->name('purchase_order.get_products');
    ...
    /* --- Purchase Entries (Manual Management) --- */
    // CRITICAL: Specific routes MUST be defined BEFORE parameterized routes to avoid conflicts
    Route::prefix('purchase-entries')->group(function () {
        // Index route (must be first to avoid conflicts)
        Route::get('/', [PurchaseProductController::class, 'purchase_entries_index'])->name('purchase-entries.index');
        
        // Specific routes (must come before {id} routes)
        Route::get('create', [PurchaseProductController::class, 'purchase_entry_create'])->name('purchase-entries.create');
        Route::post('store', [PurchaseProductController::class, 'purchase_entry_store'])->name('purchase-entries.store');
        
        // Invoice-wise operations (MUST come before {id} routes)
        Route::post('delete-invoice', [PurchaseProductController::class, 'deleteInvoice'])->name('purchase-entries.delete-invoice');
        Route::post('bulk-delete', [PurchaseProductController::class, 'bulkDeletePurchaseEntries'])->name('purchase-entries.bulk-delete');
        Route::get('invoice/{invoice}', [PurchaseProductController::class, 'viewInvoiceProducts'])->name('purchase-entries.invoice-products');
        Route::get('invoice/{invoice}/edit', [PurchaseProductController::class, 'editInvoice'])->name('purchase-entries.edit-invoice');
        Route::post('invoice/{invoice}/update', [PurchaseProductController::class, 'updateInvoice'])->name('purchase-entries.update-invoice');
        
        // Payment management routes
        Route::get('payment-modal', [PurchaseProductController::class, 'showPaymentModal'])->name('purchase-entries.payment-modal');
        Route::post('update-payment-status', [PurchaseProductController::class, 'updatePaymentStatus'])->name('purchase-entries.update-payment-status');
        
        // API Routes for batch selection
        Route::get('products-consolidated', [PurchaseProductController::class, 'getConsolidatedProducts'])->name('purchase-entries.products-consolidated');
        Route::get('by-batch/{productId}/{batch}', [PurchaseProductController::class, 'getPurchaseEntryByBatch'])->name('purchase-entries.by-batch');
        
        // Parameterized routes with specific suffixes (must come before generic {id})
        Route::get('{id}/edit', [PurchaseProductController::class, 'purchase_entry_edit'])->name('purchase-entries.edit');
        Route::get('{id}/show', [PurchaseProductController::class, 'purchase_entry_show'])->name('purchase-entries.show');
        
        // Update route - Accept both PUT and POST for better AJAX/FormData compatibility
        // POST is more reliable with FormData and file uploads
        Route::match(['PUT', 'POST'], '{id}', [PurchaseProductController::class, 'purchase_entry_update'])->name('purchase-entries.update');
        
        // Delete route (MUST be last to avoid catching specific routes)
        Route::delete('{id}', [PurchaseProductController::class, 'purchase_entry_destroy'])->name('purchase-entries.destroy');
    });
