...
class AttendanceExport implements FromCollection, WithHeadings, WithMapping, WithEvents
{
    ...
    public function collection()
    {
        ...
        $employees = EmployeeDetails::join('users', 'employee_details.user_id', '=', 'users.id')
            ->with(['designation', 'department', 'headquarter']);
        ...
        foreach ($employees as $employee) {
            ...
            $employeedata[$employee_index]['department'] = $employee->department->team_name ?? '-';
            $employeedata[$employee_index]['joining_date'] = $employee->joining_date ? $employee->joining_date->format('d-m-Y') : '-';
            $employeedata[$employee_index]['headquarters'] = $employee->headquarter->name ?? '-';
