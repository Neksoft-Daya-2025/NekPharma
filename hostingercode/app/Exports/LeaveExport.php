...
class LeaveExport implements FromCollection, WithHeadings, WithEvents
{
    ...
    {
        $leavesList = Leave::with('user', 'user.employeeDetail', 'user.employeeDetail.designation', 'user.employeeDetail.department', 'user.employeeDetail.headquarter', 'user.session', 'type')
        ...
        foreach ($leaveLists as $leavesList) {
            ...
            $employeeDetail = $leavesList->user->employeeDetail;

            $leavedata[$employee_index]['#'] = $emp_leave;
            $leavedata[$employee_index]['employee_name'] = $leavesList->user->name;
            $leavedata[$employee_index]['id'] = $leavesList->user->id;
            $leavedata[$employee_index]['employee_id'] = $employeeDetail->employee_id ?? '-';
            $leavedata[$employee_index]['designation'] = $employeeDetail->designation->name ?? '-';
            $leavedata[$employee_index]['department'] = $employeeDetail->department->team_name ?? '-';
            $leavedata[$employee_index]['joining_date'] = $employeeDetail->joining_date ? $employeeDetail->joining_date->format('d-m-Y') : '-';
            $leavedata[$employee_index]['headquarters'] = $employeeDetail->headquarter->name ?? '-';
    ...
    public function map($leavedata): array
    {
        ...
        $data[] = $leavedata['id'];
        $data[] = $leavedata['employee_id'] ?? '-';
        $data[] = $leavedata['designation'] ?? '-';
        $data[] = $leavedata['department'] ?? '-';
        $data[] = $leavedata['joining_date'] ?? '-';
        $data[] = $leavedata['headquarters'] ?? '-';
        $data[] = isset($leavedata['leave_date']) ? (is_array($leavedata['leave_date']) ? $leavedata['leave_date']['data'] : $leavedata['leave_date']) : '-';
        $data[] = $leavedata['duration'] ?? '-';
        $data[] = isset($leavedata['status']) ? (is_array($leavedata['status']) ? $leavedata['status']['data'] : ($leavedata['leave_status'] ?? '-')) : '-';
        $data[] = $leavedata['leave_type'] ?? '-';
        $data[] = $leavedata['paid'] ?? '-';
        return $data;
    }
}
