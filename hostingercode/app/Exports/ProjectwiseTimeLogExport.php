...
class ProjectwiseTimeLogExport implements FromCollection, WithMapping, WithHeadings, WithStyles, WithCustomStartCell
{
    ...
    {
        $query = ProjectTimeLog::with(['user', 'user.employeeDetail.designation', 'user.employeeDetail.department', 'user.employeeDetail.headquarter', 'project', 'task', 'breaks', 'activeBreak'])
    ...
    public function map($timelogs): array
    {
        ...
        foreach ($projectLogs as $projectId => $projectData) {
            ...
            $employeeDetail = $timelogs->first()->user->employeeDetail;
            ...
            $mappedData[] = [
                ...
                $employeeDetail->joining_date ? $employeeDetail->joining_date->format('d-m-Y') : '-',
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->getColumnDimension('E')->setWidth(18); // Date of Joining
        $sheet->getColumnDimension('F')->setWidth(20); // Headquarters
        $sheet->getColumnDimension('G')->setWidth(35); // Project Name
        $sheet->getColumnDimension('H')->setWidth(15); // Total Hours
        $sheet->getColumnDimension('I')->setWidth(15); // Total Break
