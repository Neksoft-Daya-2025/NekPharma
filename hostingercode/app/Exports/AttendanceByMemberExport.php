...
class AttendanceByMemberExport implements FromCollection, WithHeadings, WithMapping
{
    ...
    public function headings(): array
    {
        return [
            [
                ...
                __('app.hq'),
                __('modules.attendance.clock_in'),
                __('modules.attendance.clock_out'),
                __('app.status'),
                __('app.location'),
                __('app.total')
            ]
    ...
    public function collection()
    {
        ...
        $employee = \App\Models\User::with(['employeeDetail.designation', 'employeeDetail.department', 'employeeDetail.headquarter'])
            ->find($userId);
        $employeeDetail = $employee->employeeDetail ?? null;
    ...
    public function map($employeedata): array
    {
        ...
        // Get employee details (stored in class property)
        $employee = \App\Models\User::with(['employeeDetail.designation', 'employeeDetail.department', 'employeeDetail.headquarter'])
            ->find($this->userId);
        $employeeDetail = $employee->employeeDetail ?? null;
