                ...
                foreach ($users as $user) {
                    ...
                    foreach ($leaveTypes as $value) {
                        ...
                        if ($leaveQuota && $value->unused_leave == 'carry forward' && $leaveQuota->carry_forward_applied != $carryForwardAppliedYear) { // Here carry_forward_applied is the year for which the carry forward leaves are applied
                            // Unlimited accumulation: add the unused portion of the just-ended
                            // cycle to the running carry-forward pool (instead of overwriting it),
                            // so SL/EL balances keep growing year on year.
                            $leaveQuota->carry_forward_leaves = ($leaveQuota->carry_forward_leaves ?? 0) + $noOfRemainingLeaves;
