...
class RecalculateLeavesQuotas extends Command
{
    ...
    public function calculateNoOfLeavesAlloted($settings, $joiningDate, $user, $value)
    {
        ...
        if ($settings && $settings->leaves_start_from == 'joining_date') {
            ...
            // NOTE: legacy code previously added +1 to $countOfMonthsAllowed for
            // any employee who joined in the current calendar year. That was
            // harmless under the old "yearly = full quota" formula, but with
            // true monthly pro-rata accrual it double-credits the first year
            // (e.g. a 7-day-old joiner ends up with 2 months of accrual instead
            // of 1). The block has been removed; the month count already
            // includes the current partial month via line 252.
