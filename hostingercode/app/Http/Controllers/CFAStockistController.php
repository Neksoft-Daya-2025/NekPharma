...
class CFAStockistController extends AccountBaseController
{
    ...
    public function store(StoreCFAStockistRequest $request)
    {
        ...
        // Enforce: CFA can only be linked if they have this stockist's assigned HQ/Area
        if ($request->has('cfa_distributor_ids') && !empty($request->cfa_distributor_ids)) {
    ...
    private function validateCFADistributorAssignment(array $cfaDistributorIds, $stockistAreaId, $stockistHeadquarterId): bool|string
    {
        ...
        foreach ($users as $user) {
            ...
            if ($stockistAreaId && !$details->areas->contains('id', $stockistAreaId)) {
            ...
            if ($stockistHeadquarterId && !$details->headquarters->contains('id', $stockistHeadquarterId)) {
                $name = $details->company_name ?? $user->name;
