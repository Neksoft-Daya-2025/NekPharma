...
class ImportController extends Controller
{
    ...
    public function getImportProgress($name, $id)
    {
        ...
        $data = ['progress' => $progress, 'failedJobs' => $failedJobs, 'processedJobs' => $processedJobs, 'pendingJobs' => $pendingJobs, 'totalJobs' => $totalJobs];

        // Doctor import: include duplicate/new/skipped summary for the UI
        if ($name === 'DoctorImport' && $id) {
            $summary = Cache::get('doctor_import_result_' . $id);
            if ($summary) {
                $data['importSummary'] = $summary;
                // Clear cache once progress is 100% so next import doesn't show stale data
                if ($progress >= 100) {
                    Cache::forget('doctor_import_result_' . $id);
                }
            }
        }
