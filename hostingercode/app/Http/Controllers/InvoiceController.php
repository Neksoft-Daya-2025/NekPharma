...
class InvoiceController extends AccountBaseController
{
    ...
    public function updateDeliveryStatus(Request $request, $id)
    {
        try {
            ...
            // Check permissions
            $editPermission = user()->permission('edit_invoices');
            $userId = UserService::getUserId();
            
            abort_403(!(
                $editPermission == 'all'
                || ($editPermission == 'added' && ($invoice->added_by == $userId || $invoice->added_by == user()->id))
                || ($editPermission == 'owned' && $invoice->client_id == $userId)
                || ($editPermission == 'both' && ($invoice->client_id == $userId || $invoice->added_by == $userId || $invoice->added_by == user()->id))
            ));
    ...
    public function createCFAStockistInvoice()
    {
        ...
        // Determine CFA/Distributor ID
        // If admin, accountant, or FSA Executive, they need to select which CFA/Distributor is billing
        // If CFA/Distributor user, use their own ID
        if (PharmaDesignationHelper::hasFullCFAAccess()) {
            // Admin, accountant, FSA Executive, and MIS Executive can select any CFA/Distributor
            // Show all active clients (same as CFA Distributor Invoice create page)
            $this->cfaDistributors = User::without('session')
                ->join('role_user', 'role_user.user_id', '=', 'users.id')
                ->join('roles', 'roles.id', '=', 'role_user.role_id')
                ->join('client_details', 'users.id', '=', 'client_details.user_id')
            ...
            $cfaDistributorId = null; // Admin needs to select
        } else {
            // Current user is the CFA/Distributor
            $cfaDistributorId = user()->id;
            $this->cfaDistributors = collect([user()]);
        }

        // Load CFA Stockists linked to the CFA/Distributor and within CFA's assigned HQ/Area
        if ($cfaDistributorId) {
            ...
            // Admin - show all stockists
            $this->cfaStockists = CFAStockist::all();
        }

        // Load available stock from CFA/Distributor (where available_quantity > 0)
        if ($cfaDistributorId) {
            ...
            // Admin - no stock loaded until they select CFA/Distributor
            $this->products = collect([]);
        }
    ...
    public function storeCFAStockistInvoice(StoreInvoice $request)
    {
        try {
            ...
            // Store invoice items and manage stock
            $savedItemsCount = 0;
            foreach ($request->item_name as $key => $item) {
                // Skip if item name is null or empty
                if (is_null($item) || trim($item) === '') {
                ...
                // Get paid quantity (what user entered)
                $paidQty = floatval($request->quantity[$key] ?? 0);
                
                // Calculate total quantity (paid + free from scheme)
                $scheme = ($request->scheme[$key] ?? '') ? trim($request->scheme[$key]) : '';
                $totalQty = $paidQty; // Default to paid quantity
                $freeQty = 0;
                
                if ($scheme && strpos($scheme, '+') !== false) {
                    // Scheme format: "20+2" means 20 paid + 2 free
                ...
                $invoiceItem = InvoiceItems::create([
                    ...
                    'item_summary' => $request->item_summary[$key] ?? null,
                    'type' => 'item',
                    'quantity' => $paidQty, // Store paid quantity in invoice item
                    'unit_price' => round($request->cost_per_item[$key] ?? 0, 2),
                    'amount' => round($request->amount[$key] ?? 0, 2),
                    'taxes' => isset($request->taxes[$key]) && !empty($request->taxes[$key]) ? json_encode($request->taxes[$key]) : null,
                    'hsn_sac_code' => $hsnSacCode,
                    'product_id' => $request->product_id[$key] ?? null,
                    'unit_id' => $request->unit_id[$key] ?? null,
                    'purchase_entry_id' => null, // Not used for CFA/Stockist invoices
                    'scheme' => $scheme,
                    'pack' => $request->pack[$key] ?? null,
                    'mfr' => $request->mfr[$key] ?? null,
                    'batch' => $request->batch[$key] ?? null,
                ...
                // Process stock: Reduce from CFA/Distributor stock and create Stockist stock
                if (!empty($request->cfa_distributor_stock_id[$key])) {
                    ...
                    if ($distributorStock) {
                        ...
                        if ($distributorStock->available_quantity < $totalQty) {
                            ...
                            return Reply::error('Insufficient stock available. Required: ' . $totalQty . ', Available: ' . $distributorStock->available_quantity);
                        }
                        
                        // Reduce available quantity from CFA/Distributor stock
                        $distributorStock->available_quantity -= $totalQty;
                        $distributorStock->save();
                        
                        \Log::info('Reduced CFA/Distributor stock', [
                            'distributor_stock_id' => $distributorStock->id,
                            'reduced_qty' => $totalQty,
                            'remaining_qty' => $distributorStock->available_quantity,
                            'paid_qty' => $paidQty,
                            'free_qty' => $freeQty
                        ]);
                        
                        // Create CFA Stockist stock entry
                        CFAStockistStock::create([
                            ...
                            'quantity' => $totalQty, // Store total quantity (paid + free)
                        ...
                        \Log::info('Created CFA Stockist stock entry', [
                            'stockist_stock_id' => CFAStockistStock::latest()->first()->id,
                            'cfa_stockist_id' => $request->cfa_stockist_id,
                            'quantity' => $totalQty
                        ]);
                    } else {
                        ...
                        return Reply::error('Stock entry not found for item: ' . $item);
                    }
                }
            }
    ...
    {
        abort_403(!$this->userCanEditCfaStockistInvoice());

        // TODO: Implement full edit functionality
        return Reply::error('CFA/Stockist Invoice editing is not yet implemented. Please contact support.');
    }

    public function updateCFAStockistInvoice(UpdateInvoice $request, $id)
    {
        abort_403(!$this->userCanEditCfaStockistInvoice());

        // TODO: Implement full update functionality
        return Reply::error('CFA/Stockist Invoice update is not yet implemented. Please contact support.');
    }
    ...
    /**
     * Same baseline as add; edit UI is not implemented yet but gate stays aligned.
     */
    private function userCanEditCfaStockistInvoice(): bool
    {
        if (PharmaDesignationHelper::hasFullCFAAccess()) {
            return true;
        }
