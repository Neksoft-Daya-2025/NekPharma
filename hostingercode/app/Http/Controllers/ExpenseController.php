...
class ExpenseController extends AccountBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $expensesQuery = Expense::with(['user', 'submittedTo', 'approver', 'headquarter'])
    ...
    public function status(Request $request)
    {
        ...
        // Load expenses with approval details (only pharma expenses)
        $expensesQuery = Expense::with(['user', 'submittedTo', 'approver'])
            ->where('expense_type', 'pharma_statement')
            ->where('company_id', company()->id);
        ...
        // Admin can see all expenses or filter by employee
        if ($this->viewPermission == 'all') {
            if (!user()->hasRole('admin')) {
                $viewableIds = RoleHierarchy::userIdsViewableBy(user(), company()->id);
                if (!empty($viewableIds)) {
            ...
            // Non-admin: Show only expenses created by current user
