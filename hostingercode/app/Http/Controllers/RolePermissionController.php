...
class RolePermissionController extends AccountBaseController
{
    ...
    public function permissions()
    {
        ...
        if ($this->role->name == 'client') {
            ...
            $this->modulesData = Module::with(['permissions' => function($query) {
                $query->where('is_custom', 0)->orderBy('id', 'asc');
            ...
            $this->modulesData = Module::with(['permissions' => function($query) {
                $query->where('is_custom', 0)->orderBy('id', 'asc');
            }])->where('module_name', '<>', 'messages')->withCount('customPermissions')->get();
            
            // Ensure permissions are loaded for all modules
            foreach ($this->modulesData as $module) {
                if (!$module->relationLoaded('permissions')) {
            ...
            // Determine which role type to use for ModuleSettings
            // For 'employee' and 'client', use their own type
            // For custom roles, check if ModuleSettings exist for that role, otherwise fall back to 'employee'
            $roleType = in_array($this->role->name, ['employee', 'client']) ? $this->role->name : 'employee';
            
            // Check if custom role has its own ModuleSettings
            if (!in_array($this->role->name, ['employee', 'client', 'admin'])) {
                $customRoleSettings = ModuleSetting::where('type', $this->role->name)
                    ->where('company_id', company()->id)
                    ->count();
                
                // If custom role has its own settings, use them
                if ($customRoleSettings > 0) {
                    $roleType = $this->role->name;
                }
            }
            ...
            $this->employeeModules = array_values(array_unique(array_merge(
                ...
                ModuleSetting::pharmaFinanceMatrixUnlockModules($this->role->name)
            )));
    ...
    /**
     * Update permissions for a user in a role.
     *
     * @return \Illuminate\Http\Response
     */
    ...
    /**
     * Handle custom permissions request (add/edit custom permission).
     *
     * @return \Illuminate\Http\Response
     */
