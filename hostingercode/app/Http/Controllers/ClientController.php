...
class ClientController extends AccountBaseController
{
    ...
    /**
     * Show the form for creating a new client (optionally from a lead).
     *
     * @return \Illuminate\Http\Response
     */
    ...
    public function store(StoreClientRequest $request)
    {
        ...
        $data['login'] = $request->login ?? 'enable';
        ...
        $clientDetails = $user->clientDetails()->create($data);
        $client_id = $user->id;
        
        // Sync areas if provided
        if ($request->has('area_ids') && is_array($request->area_ids)) {
            $clientDetails->areas()->sync($request->area_ids);
        }

        // Sync headquarters if provided (CFA/Distributor assigned HQ)
        if ($request->has('headquarter_ids') && is_array($request->headquarter_ids)) {
            $clientDetails->headquarters()->sync($request->headquarter_ids);
        }
    ...
    {
        $this->client = User::withoutGlobalScope(ActiveScope::class)->with('clientDetails.areas', 'clientDetails.headquarters')->findOrFail($id);
    ...
    public function update(UpdateClientRequest $request, $id)
    {
        ...
        unset($data['country']);
        unset($data['is_client_contact']);
        unset($data['password']); // Always unset password from data array
        ...
        if ($request->has('password') && $request->password !== null && trim($request->password) !== '') {
            $passwordToUpdate = \Hash::make(trim($request->password));
            \Log::info('Client password update', [
                'user_id' => $id,
        ...
        // Update password separately using forceFill to ensure it's saved
        if ($passwordToUpdate !== null) {
            // Get old password hash for comparison
            $oldPasswordHash = $user->password;
            
            \Log::info('Client password update - Before save', [
                'user_id' => $id,
                'user_email' => $user->email,
                'old_password_hash' => substr($oldPasswordHash, 0, 20) . '...',
                'new_password_hash' => substr($passwordToUpdate, 0, 20) . '...',
            ]);
            
            // Update password using forceFill
            $user->forceFill([
                'password' => $passwordToUpdate
            ])->save();
            
            // Verify password was saved by querying database directly
            $userFromDb = \DB::table('users')->where('id', $id)->first();
            $dbPasswordHash = $userFromDb->password ?? null;
            
            \Log::info('Client password update - After save', [
                'user_id' => $id,
                'user_email' => $user->email,
                'password_updated' => ($oldPasswordHash !== $dbPasswordHash),
                'old_hash' => substr($oldPasswordHash, 0, 20) . '...',
                'new_hash_in_db' => $dbPasswordHash ? substr($dbPasswordHash, 0, 20) . '...' : 'NULL',
                'password_saved_correctly' => ($passwordToUpdate === $dbPasswordHash),
            ]);
        ...
        if ($user->clientDetails) {
            ...
            // Sync areas if provided
            if ($request->has('area_ids') && is_array($request->area_ids)) {
                $user->clientDetails->areas()->sync($request->area_ids);
            } else {
                // If no areas provided, clear existing areas
                $user->clientDetails->areas()->detach();
            }

            // Sync headquarters if provided (CFA/Distributor assigned HQ)
            if ($request->has('headquarter_ids') && is_array($request->headquarter_ids)) {
                $user->clientDetails->headquarters()->sync($request->headquarter_ids);
            } else {
                $user->clientDetails->headquarters()->detach();
            }
    ...
    /**
     * Apply quick action to selected clients.
     *
     * @return \Illuminate\Http\Response
     */
    ...
    /**
     * Get project chart data for the client.
     *
     * @return array
     */
    ...
    /**
     * Get invoice chart data for the client.
     *
     * @return array
     */
    ...
    /**
     * Return client details for AJAX requests.
     *
     * @return \Illuminate\Http\Response
     */
