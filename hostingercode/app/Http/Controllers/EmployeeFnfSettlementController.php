...
class EmployeeFnfSettlementController extends AccountBaseController
{
    ...
    {
        // Load all employee data with all necessary relationships
        $this->fnfSettlement = EmployeeFnfSettlement::with([
            'employee.employeeDetail.designation',
            'employee.employeeDetail.department',
            'employee.employeeDetail.reportingTo',
            'employee.employeeDetail.headquarter',
            'employee.employeeDetail.exstation',
            'employee.employeeDetail.outstation',
            'employee.employeeDetail.area',
            'approvedBy',
            'addedBy'
        ])->findOrFail($id);
    ...
    {
        // EL (Earned Leave) balance only: use leave types marked encashable or named Earned Leave/EL
        $leaveBalance = 0;

        foreach ($employee->leaveTypes as $leaveQuota) {
            $leaveType = $leaveQuota->leaveType;
            if (!$leaveType || $leaveQuota->leaves_remaining <= 0) {
                continue;
            }
            $isEncashable = (int) $leaveType->encashed === 1
                || stripos($leaveType->type_name ?? '', 'earned') !== false
                || preg_match('/\bEL\b/i', $leaveType->type_name ?? '');
            if (!$isEncashable) {
                continue;
            }
        ...
        // EL encashment (daily salary * EL balance days)
        if ($fnf->basic_salary > 0 && $leaveBalance > 0) {
            $dailyRate = $fnf->basic_salary / 30;
            $fnf->leave_encashment_amount = $dailyRate * $leaveBalance;
        } else {
            $fnf->leave_encashment_amount = 0;
        }
    }
    ...
    {
        // Load all employee data with all necessary relationships
        $fnf = EmployeeFnfSettlement::with([
            'employee.employeeDetail.designation',
            'employee.employeeDetail.department',
            'employee.employeeDetail.reportingTo',
            'employee.employeeDetail.headquarter',
