...
class CreditNoteController extends AccountBaseController
{
    ...
    public function store(StoreCreditNotes $request)
    {
        ...
        $creditNote->credit_note_type = in_array($request->credit_note_type, ['saleable', 'non_saleable']) ? $request->credit_note_type : 'saleable';
        $creditNote->save();
    ...
    public function update(UpdateCreditNote $request, $id)
    {
        ...
        $creditNote->adjustment_amount = round($request->adjustment_amount, 2);
        $creditNote->note = trim_editor($request->note);
        if (in_array($request->credit_note_type ?? '', ['saleable', 'non_saleable'])) {
            $creditNote->credit_note_type = $request->credit_note_type;
        }
        $creditNote->save();
    ...
    /**
     * Saleable vs Non-Saleable product report (SRS 3.4).
     */
    public function saleableVsNonSaleableReport(Request $request)
    {
        $viewPermission = user()->permission('view_invoices');
        abort_403(!in_array($viewPermission, ['all', 'added', 'owned', 'both']));

        $companyId = company()->id;
        $fromDate = $request->filled('from_date') ? Carbon::parse($request->from_date)->startOfDay() : Carbon::now()->startOfMonth();
        $toDate = $request->filled('to_date') ? Carbon::parse($request->to_date)->endOfDay() : Carbon::now()->endOfDay();

        $saleable = DB::table('credit_note_items')
            ->join('credit_notes', 'credit_notes.id', '=', 'credit_note_items.credit_note_id')
            ->leftJoin('products', 'products.id', '=', 'credit_note_items.product_id')
            ->where('credit_notes.company_id', $companyId)
            ->whereBetween('credit_notes.issue_date', [$fromDate, $toDate])
            ->where(function ($q) {
                $q->where('credit_notes.credit_note_type', 'saleable')
                    ->orWhereNull('credit_notes.credit_note_type');
            })
            ->where('credit_note_items.type', 'item')
            ->select(
                'credit_note_items.product_id',
                DB::raw('MAX(COALESCE(products.name, credit_note_items.item_name)) as product_name'),
                DB::raw('SUM(credit_note_items.amount) as total_amount'),
                DB::raw('SUM(credit_note_items.quantity) as total_quantity')
            )
            ->groupBy('credit_note_items.product_id')
            ->get();

        $nonSaleable = DB::table('credit_note_items')
            ->join('credit_notes', 'credit_notes.id', '=', 'credit_note_items.credit_note_id')
            ->leftJoin('products', 'products.id', '=', 'credit_note_items.product_id')
            ->where('credit_notes.company_id', $companyId)
            ->whereBetween('credit_notes.issue_date', [$fromDate, $toDate])
            ->where('credit_notes.credit_note_type', 'non_saleable')
            ->where('credit_note_items.type', 'item')
            ->select(
                'credit_note_items.product_id',
                DB::raw('MAX(COALESCE(products.name, credit_note_items.item_name)) as product_name'),
        ...
        $keys = $saleable->pluck('product_id')->map(fn ($id) => (string) $id)->merge(
            $nonSaleable->pluck('product_id')->map(fn ($id) => (string) $id)
        )->unique()->values();
        $rows = [];
        foreach ($keys as $key) {
            $pid = $key === '' ? null : (int) $key;
            $s = $saleable->first(fn ($x) => (string) $x->product_id === $key);
            $n = $nonSaleable->first(fn ($x) => (string) $x->product_id === $key);
            $rows[] = [
                'product_id' => $pid,
                'product_name' => $s ? $s->product_name : ($n ? $n->product_name : '-'),
                'saleable_amount' => $s ? (float) $s->total_amount : 0,
                'saleable_quantity' => $s ? (float) $s->total_quantity : 0,
                'non_saleable_amount' => $n ? (float) $n->total_amount : 0,
                'non_saleable_quantity' => $n ? (float) $n->total_quantity : 0,
            ];
        }
        usort($rows, fn ($a, $b) => strcmp((string) $a['product_name'], (string) $b['product_name']));

        $this->reportRows = $rows;
        $this->fromDate = $fromDate->format('Y-m-d');
        $this->toDate = $toDate->format('Y-m-d');
        $this->filterFromDate = $request->from_date;
        $this->filterToDate = $request->to_date;

        return view('credit-notes.saleable-vs-non-saleable-report', $this->data);
    }
