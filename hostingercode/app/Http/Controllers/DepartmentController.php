    ...
    public function getMembers($id)
    {
        ...
        if ($id == 0) {
            ...
            foreach ($members as $item) {
                $options .= EmployeeSelectLabel::bootstrapSelectOptionHtml($item, user() ? user()->id : null);
            }
        }
        else {
            $members = collect([]);
            ...
            $userModels = User::with('employeeDetail.designation')
                ->whereIn('id', $members->pluck('id'))
                ->get()
                ->keyBy('id');

            foreach ($members as $item) {
                $u = $userModels->get($item->id);
                if (! $u) {
                    continue;
                }

                $isSelected = isset($userId) && in_array($item->id, $userId);
                $options .= EmployeeSelectLabel::bootstrapSelectOptionHtml($u, user() ? user()->id : null, $isSelected);
                $url = route('employees.show', [$item->id]);

                $userData[] = ['id' => $item->id, 'value' => EmployeeSelectLabel::plain($u), 'image' => $u->image_url, 'link' => $url];
