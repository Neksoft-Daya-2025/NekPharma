...
use App\Models\AttendanceMonthFinalisation;
...
class AttendanceController extends AccountBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $monthNum = (int) ($request->input('month') ?? $now->format('m'));
        ...
        $this->month = str_pad((string) $monthNum, 2, '0', STR_PAD_LEFT);
        $this->departments = Team::all();
        $this->designations = Designation::all();
        // SRS 3.1.3: For "Finalise attendance" button – whether the selected month is already finalised
        $this->attendanceFinalisedForCurrentMonth = AttendanceMonthFinalisation::isFinalised(company()->id, $year, $monthNum);
    ...
    public function summaryData($request)
    {
        ...
        $employees = User::with(
            ...
            ->select('users.id', 'users.name', 'users.inactive_date', 'users.email', 'users.created_at', 'employee_details.department_id', 'employee_details.user_id', 'employee_details.added_by', 'employee_details.employee_id', 'employee_details.headquarter_id', 'users.image')
            ->onlyEmployee()
        ...
        $employeeRowMeta = [];
        foreach ($employees as $employee) {
            $rowKey = $employee->id.'#'.$employee->name;
            $detail = $employee->employeeDetail;
            $employeeRowMeta[$rowKey] = [
                'employee_id' => $detail && $detail->employee_id ? $detail->employee_id : '',
                'headquarter' => ($detail && $detail->headquarter) ? $detail->headquarter->name : '',
            ];
        }
        $this->employeeRowMeta = $employeeRowMeta;
    ...
    /**
     * Show attendance details for the given ID.
     *
    ...
    {
        $attendance = Attendance::findOrFail($id);
        // SRS 3.1.3: Field employees – no manual attendance edit
        if ($attendance->user && $attendance->user->isFieldEmployee()) {
            return Reply::error(__('messages.attendanceFieldEmployeeNoManual'));
        }
    ...
    {
        // SRS 3.1.3: Field employees – no manual add attendance
        $targetUser = User::find($request->user_id);
        if ($targetUser && $targetUser->isFieldEmployee()) {
            return Reply::error(__('messages.attendanceFieldEmployeeNoManual'));
        }
        ...
        if ($attendance && $request->user_id) {
            ...
            foreach ($attendances as $item) {
                $itemIn = $item->clock_in_time ? $item->clock_in_time->timezone($this->company->timezone) : null;
                $itemOut = $item->clock_out_time ? $item->clock_out_time->timezone($this->company->timezone) : null;

                // Treat an open (clock-in without clock-out) record on the same date as
                // an already-marked attendance — the user must edit that record instead.
                if ($itemIn && $itemOut === null) {
                    return Reply::error(__('messages.attendanceMarked'));
                }

                // If we don't have both sides of the existing interval, nothing reliable
                // to overlap-check against, so skip this row.
                if (!$itemIn || !$itemOut) {
                    continue;
                }

                $overlapsExisting = !$itemIn->lt($clockIn)
                    && $itemOut->gt($clockIn)
                    && ($clockOut === null || !$itemIn->lt($clockOut));

                if (!$overlapsExisting && !$itemOut->lt($clockIn)) {
                    return Reply::error(__('messages.attendanceMarked'));
                }
            }
        }
    ...
    /**
     * Show attendance by member view.
     *
     * @return \Illuminate\Http\Response
     */
    ...
    /**
     * Bulk mark attendance for selected employees/dates.
     *
     * @return \Illuminate\Http\Response
     */
    ...
    /**
     * SRS 3.1.3: Finalise monthly attendance so payroll can be generated for that month.
     */
    public function finaliseMonth(Request $request)
    {
        abort_403($this->viewAttendancePermission != 'all');
        ...
        $existing = AttendanceMonthFinalisation::where('company_id', company()->id)
            ->where('year', $year)
            ->where('month', $month)
            ->first();

        if ($existing && $existing->finalised_at) {
            return Reply::success(__('messages.attendanceAlreadyFinalised'));
        }

        AttendanceMonthFinalisation::updateOrCreate(
            [
                'company_id' => company()->id,
                'year' => $year,
                'month' => $month,
        ...
        return Reply::success(__('messages.attendanceFinalisedSuccess'));
    }
    ...
    public function qrClockInOut(Request $request, $hash)
    {
        ...
        if ($todayAttendance && !session('qr_clock_in')) {
            // SRS 3.1.3: Field employees – no manual clock-out
            if ($user->isFieldEmployee()) {
                return redirect()->route('dashboard')->with('error', __('messages.attendanceFieldEmployeeNoManual'));
            }
            ...
            if ($attendanceSetting->qr_enable == 1) {
                // SRS 3.1.3: Field employees – no manual clock-in
                if ($user->isFieldEmployee()) {
                    return view('attendance-settings.ajax.qrview', [
                        'message' => __('messages.attendanceFieldEmployeeNoManual'),
                        'intimeDate' => null,
                        'outtimeDate' => null,
                        'totalWorkingTime' => null,
    ...
    {
        // SRS 3.1.3: Field employees – attendance only via DCR Close Day
        if ($user->isFieldEmployee()) {
            return ['type' => 'error', 'message' => __('messages.attendanceFieldEmployeeNoManual')];
        }
