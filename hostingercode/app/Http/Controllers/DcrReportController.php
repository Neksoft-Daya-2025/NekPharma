...
class DcrReportController extends AccountBaseController
{
    ...
    private function buildDcrFormData(?DcrReport $editing): void
    {
        ...
        if ($accessibleHqIds !== null && ! user()->hasRole('admin')) {
        ...
        if ($editing !== null) {
            ...
            $this->canAddForOthers = user()->hasRole('admin') || $this->addPermission === 'all';
            $this->employeesForDropdown = collect();
    ...
    public function getDcrContextForEmployee(Request $request)
    {
        ...
        $canAddForOthers = user()->hasRole('admin') || $addPermission === 'all';
    ...
    public function storeVisit(Request $request)
    {
        ...
        $dcrOwnerId = user()->id;
        $canAddForOthers = user()->hasRole('admin') || $this->addPermission === 'all';
    ...
    private function syncTourWithDraftIfNeeded(DcrReport $draft): void
    {
        ...
        $tourLockedAfterApproval = $tour->approved && ! user()->hasRole('admin');

        if ($tourLockedAfterApproval || ($monthLocked && ! user()->hasRole('admin'))) {
