...
class PermissionRole extends BaseModel
{
    ...
    /**
     * Get the permission type for this role-permission.
     *
     * @return BelongsTo
     */
    ...
    /**
     * Modules whose permissions are set to type {@see PermissionType::ALL} for the `accountant` role
     * (clients/estimates/invoices/payments/expenses/banking/orders/contracts/products/CFA/SFC/stockists),
     * plus finance reports and core finance-related settings.
     *
     * @var array<int, string>
     */
    private const ACCOUNTANT_FINANCE_MODULES = [
        'clients',
        'estimates',
        'invoices',
        'payments',
    ...
    /**
     * Extra permission names (reports + finance settings) not fully covered by module loop.
     *
     * @var array<int, string>
     */
    private const ACCOUNTANT_EXTRA_FINANCE_PERMISSIONS = [
        'view_finance_report',
        'view_income_expense_report',
        'view_expense_report',
        'view_sales_report',
        'manage_currency_setting',
        'manage_payment_setting',
        'manage_finance_setting',
    ];

    /**
     * Permission names that should default to "all" for the accountant role (finance department scope).
     *
     * @return array<int, string>
     */
    public static function accountantFinancePermissionNames(): array
    {
        $names = [];
        foreach (Module::MODULE_LIST as $module) {
            if (! in_array($module['module_name'], self::ACCOUNTANT_FINANCE_MODULES, true)) {
                continue;
            }
        ...
        return array_values(array_unique(array_merge($names, self::ACCOUNTANT_EXTRA_FINANCE_PERMISSIONS)));
    }

    /**
     * Set permission_type to "all" on the accountant role for finance-related permissions, then
     * re-sync {@see UserPermission} for users on that role when they use default (non-custom) permissions.
     *
     * @return int Number of accountant roles updated
     */
    public static function syncAccountantRoleFinancePermissions(?int $companyId = null): int
    {
        $names = self::accountantFinancePermissionNames();
        $permissionIds = Permission::query()->whereIn('name', $names)->pluck('id');
        if ($permissionIds->isEmpty()) {
            return 0;
        }

        $roles = Role::query()->where('name', 'accountant');
        if ($companyId !== null) {
        ...
        foreach ($roles->with('users')->get() as $role) {
            ...
            foreach ($role->users as $user) {
                if ((int) $user->customised_permissions === 0) {
