...
class ModuleSetting extends BaseModel
{
    ...
    /** Pharma field force modules: enabled for admin and employee like OTHER_MODULES. */
    ...
    /**
     * SFC/CFA are often disabled for generic "employee" panel users (field force) in Module Settings,
     * but finance roles still need to edit those permission rows in Roles & Permissions / HR permissions.
     *
     * @return array<int, string> Extra module_name values merged into the permission-matrix allowlist
     */
    public static function pharmaFinanceMatrixUnlockModules(?string $roleName): array
    {
        if ($roleName === null || $roleName === 'client') {
            return [];
        }

        $unlockForRoles = ['accountant', 'fsa-executive'];

        if (in_array($roleName, $unlockForRoles, true)) {
            return ['sfc', 'cfa'];
        }
