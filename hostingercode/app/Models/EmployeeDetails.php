...
class EmployeeDetails extends BaseModel
{
    ...
    {
        return $this->belongsTo(\App\Models\PharmaHeadquarter::class, 'headquarter_id');
    }

    /**
     * Generate the next employee ID in the format "RVB / 100", "RVB / 101", etc.
     * 
     * @param int|null $companyId
     * @return string
     */
    public static function generateEmployeeId($companyId = null): string
    {
        $companyId = $companyId ?? company()->id;
        
        // Get the prefix from company settings (Requirement 3.1.1; default to 'RVB')
        $company = $companyId ? Company::find($companyId) : null;
        $prefix = $company && !empty($company->employee_id_prefix) ? $company->employee_id_prefix : 'RVB';
        
        // Count existing employees for this company
        $employeeCount = self::where('company_id', $companyId)->count();
        
        // Generate the next employee ID starting from 100
        $nextNumber = 100 + $employeeCount;
