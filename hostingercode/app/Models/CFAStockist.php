...
class CFAStockist extends BaseModel
{
    ...
    public static function generateCFAStockistId($companyId = null): string
    {
        ...
        // Get all existing CFA Stockist IDs for this company (including soft-deleted)
        // This ensures we don't reuse IDs from deleted records
        ...
        $attempts = 0;
        while (self::withTrashed()->where('company_id', $companyId)->where('cfa_stockist_id', $newId)->exists() && $attempts < 10) {
            $nextNumber++;
            $formattedNumber = str_pad($nextNumber, 3, '0', STR_PAD_LEFT);
            $newId = $prefix . '-' . $formattedNumber;
            $attempts++;
