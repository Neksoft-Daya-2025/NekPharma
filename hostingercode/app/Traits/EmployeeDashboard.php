...
trait EmployeeDashboard
{
    ...
    public function employeeDashboard()
    {
        ...
        // Tour Plan: show approved status on employee dashboard (3.2.4)
        $this->approvedTourPlanSummary = null;
        if (in_array('tours', $this->user->modules ?? [])) {
            $approvedQuery = \App\Models\Tour::where('user_id', $this->user->id)
                ->where('company_id', company()->id)
                ->where(function ($q) {
                    $q->where('approved', true)->orWhere('status', 'approved');
                });
            $currentMonth = now(company()->timezone)->format('Y-m');
            $currentMonthApproved = (clone $approvedQuery)->whereYear('date', now(company()->timezone)->year)
                ->whereMonth('date', now(company()->timezone)->month)->count();
            $latestApproved = (clone $approvedQuery)->orderByDesc('date')->first();
            $this->approvedTourPlanSummary = [
                'current_month_count' => $currentMonthApproved,
                'current_month' => $currentMonth,
                'has_approved' => (clone $approvedQuery)->exists(),
                'latest_approved_month_label' => $latestApproved ? $latestApproved->date->format('F Y') : null,
            ];
        }
