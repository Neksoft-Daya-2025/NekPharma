...
class ImportStockistJob implements ShouldQueue
{
    ...
    private function processStockistRow($shopname, $ownerName, $ownerMobileRaw, $headquarterName, $stationType, $stationName, $address, $companyId, $headquarters, $exstations, $outstations, $existingStockistsByEmail, $existingStockistsByMobile, $existingStockistsByShopnameHq)
    {
        ...
        if ($this->allowedHeadquarterIds !== null && !in_array($headquarter->id, $this->allowedHeadquarterIds, true)) {
            $msg = 'Stockist import skipped for "' . $shopname . '": Headquarter "' . $headquarterName . '" is not in your assigned areas.';
            \Log::warning($msg);
        ...
        if ($this->isColumnExists('msl_number')) {
            ...
            if (!empty($mslNumber)) {
                // Check if MSL number already exists in doctors, chemists, or stockists (excluding current stockist)
                $mslExists = $this->mslNumberExists($mslNumber, $companyId, $stockist->id ?? null);
                if ($mslExists) {
                    \Log::warning('Stockist import: MSL number "' . $mslNumber . '" already exists for stockist "' . $shopname . '"');
                    // Don't throw exception, just skip MSL number assignment
                } else {
                    $stockist->msl_number = $mslNumber;
                }
            }
        }
     ...
     */
    private function mslNumberExists(string $mslNumber, int $companyId, ?int $excludeStockistId = null): bool
    {
        ...
        $stockistQuery = Stockist::where('msl_number', $mslNumber)
            ->where('company_id', $companyId);
        if ($excludeStockistId) {
            $stockistQuery->where('id', '!=', $excludeStockistId);
        }
