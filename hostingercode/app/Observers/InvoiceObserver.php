...
class InvoiceObserver
{
    ...
    {
        if (!isRunningInConsoleOrSeeding()) {
            // Skip item creation for CFA/Distributor and CFA/Stockist invoices - they are handled by the controller
            ...
            // Check if this is a CFA/Stockist invoice
            $isCFAStockistInvoice = request()->routeIs('cfa-stockist-invoices.store') 
                || request()->routeIs('cfa-stockist-invoices.update')
                || request()->has('invoice_type') && request()->invoice_type === 'cfa_stockist'
                || str_contains(request()->url(), 'cfa-stockist-invoices');
            
            if ($isCFADistributorInvoice || $isCFAStockistInvoice) {
                // CFA/Distributor and CFA/Stockist invoices handle items in the controller, skip observer
                return;
            }
                    ...
                    if (!is_null($item)) {
                        $invoiceItem = InvoiceItems::create(
                            [
                                ...
                                'unit_price' => isset($cost_per_item[$key]) ? round($cost_per_item[$key], 2) : 0,
                                'amount' => isset($amount[$key]) ? round($amount[$key], 2) : 0,
                                'taxes' => ($tax && is_array($tax) && array_key_exists($key, $tax)) ? json_encode($tax[$key]) : null,
                                'field_order' => $key + 1
                            ]
                        );
                    }
    ...
    {
        if (!isRunningInConsoleOrSeeding()) {
            // Skip item updates for CFA/Distributor and CFA/Stockist invoices - they are handled by the controller
            // Check if this is a CFA/Distributor invoice by checking the route or request
            $isCFADistributorInvoice = request()->routeIs('cfa-distributor-invoices.store') 
                || request()->routeIs('cfa-distributor-invoices.update')
                || request()->has('invoice_type') && request()->invoice_type === 'cfa_distributor'
    ...
    public function deleting(Invoice $invoice)
    {
        ...
        /* Delete CFA Distributor Stock entries and restore main product stock */
        // When CFA Distributor invoice is deleted, restore stock to PurchaseStockAdjustment
        // Stock was reduced when invoice was created, so we need to restore it on deletion
        $cfaStocks = \App\Models\CFADistributorStock::where('invoice_id', $invoice->id)->get();
        if ($cfaStocks && $cfaStocks->count() > 0) {
            $hasAdjustmentTable = Schema::hasTable('purchase_stock_adjustments');
            $hasBatchStockTable = Schema::hasTable('purchase_batch_stock');
            $companyId = company()?->id;

            \Log::info('Deleting CFA Distributor Stock entries and restoring product stock for invoice deletion', [
                'invoice_id' => $invoice->id,
                'stock_entries_count' => $cfaStocks->count(),
                'has_purchase_stock_adjustments' => $hasAdjustmentTable,
                'has_purchase_batch_stock' => $hasBatchStockTable,
            ]);

            foreach ($cfaStocks as $stock) {
                try {
                    if ($stock->product_id && $stock->quantity > 0 && $companyId) {
                        if ($hasAdjustmentTable) {
                            $stockAdjustment = PurchaseStockAdjustment::where('product_id', $stock->product_id)
                                ->where('company_id', $companyId)
                                ->first();

                            if ($stockAdjustment) {
                                $stockAdjustment->net_quantity += $stock->quantity;
                                $stockAdjustment->save();

                                \Log::info('Restored product stock on invoice deletion', [
                                    'invoice_id' => $invoice->id,
                                    'product_id' => $stock->product_id,
                                    'quantity_restored' => $stock->quantity,
                                    'new_stock' => $stockAdjustment->net_quantity,
                                ]);
                            }
                        }

                        if ($hasBatchStockTable) {
                            ...
                            $batchStock->quantity = (float) $batchStock->quantity + (float) $stock->quantity;
                            $batchStock->save();
                        }
                    }
                } catch (\Throwable $e) {
                    \Log::error('CFA distributor stock restore failed during invoice delete (CFA row will still be removed)', [
                        'invoice_id' => $invoice->id,
                ...
                \Log::info('Deleting CFA stock entry', [
                    'stock_id' => $stock->id,
                    'product_id' => $stock->product_id,
