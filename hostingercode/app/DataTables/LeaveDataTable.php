        ...
        return datatables()
            ...
            ->addColumn('status', function ($row) {
                if ($row->status == 'approved') {
                    ...
                    $class = 'text-info';
                    $status = __('modules.leaves.managerApproved');
            ...
            ->addColumn('action', function ($row) {
                ...
                $isReportee = $this->reportingTo->pluck('user_id')->contains($row->user_id);

                // Two-step flow: manager_approved → only HR (permission all) can final Approve/Reject
                if ($row->status == 'manager_approved' && ($row->duration != 'multiple' || is_null($row->unique_id)) && $this->approveRejectPermission == 'all') {
                    ...
                    if (!$rejectActionAdded) {
                        ...
                        $rejectActionAdded = true;
                    }
                }

                if ($row->status == 'manager_approved' && $row->duration == 'multiple' && !is_null($row->unique_id) && $this->approveRejectPermission == 'all') {
                    if (!$approvedActionAdded) {
                        $actions .= '<a class="dropdown-item leave-action-approved" data-leave-id=' . $row->unique_id . '
                    ...
                    if (!$rejectActionAdded) {
                        $actions .= '<a data-leave-id=' . $row->unique_id . ' data-type="approveAll"
                                 data-leave-action="rejected" data-user-id="' . $row->user_id . '" data-leave-type-id="' . $row->leave_type_id . '" class="dropdown-item leave-action-reject" href="javascript:;">
                                    <i class="fa fa-times mr-2"></i>' . __('app.reject') . '</a>';
                        $rejectActionAdded = true;
                    }
                ...
                if (($row->duration == 'multiple' && !is_null($row->unique_id)) && ($this->approveRejectPermission == 'all' || $isReportee)) {
                ...
                // Pending: Reporting Manager can Approve (→ manager_approved). HR /
                // admin can Approve in a single step (→ approved) — see LeaveController@leaveStore.
                $isAdminOrHR = user()->hasRole('admin') || $this->approveRejectPermission == 'all';
                if ($row->status == 'pending' && $row->user_id != user()->id && !($this->reportingPermission == 'cannot-approve')) {
                    if (!$rejectActionAdded && ($isReportee || $isAdminOrHR)) {
                        $actions .= '<a data-leave-id=' . $row->id . '
                                 data-leave-action="rejected" data-user-id="' . $row->user_id . '" data-leave-type-id="' . $row->leave_type_id . '" class="dropdown-item leave-action-reject" href="javascript:;">
                    ...
                    if (!$approvedActionAdded && (
                            $isAdminOrHR
                            || ($isReportee && $row->manager_status_permission == '' && $this->reportingPermission == 'approved')
                        )) {
                        ...
                        $approvedActionAdded = true;
                    } elseif (!$approvedActionAdded && $isReportee && $this->reportingPermission == 'pre-approve' && !$row->manager_status_permission) {
                        $actions .= '<a data-leave-id=' . $row->id . '
                ...
                if (
                    ...
                    if (!in_array($row->status, ['approved'], true)) {
