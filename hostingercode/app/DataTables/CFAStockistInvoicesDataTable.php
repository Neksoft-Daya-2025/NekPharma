...
class CFAStockistInvoicesDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('action', function ($row) use ($firstInvoice, $userId) {
            ...
            // Edit action hidden until CFA Stockist invoice edit/update is implemented (see InvoiceController::editCFAStockistInvoice)

            if ($this->deleteInvoicePermission == 'all' || ($this->deleteInvoicePermission == 'added' && ($userId == $row->added_by || user()->id == $row->added_by)) || ($this->deleteInvoicePermission == 'owned' && $userId == $row->client_id) || ($this->deleteInvoicePermission == 'both' && ($userId == $row->added_by || user()->id == $row->added_by || $userId == $row->client_id))) {
        ...
        $datatables->addColumn('stockist_name', function ($row) {
            return $row->cfaStockist ? $row->cfaStockist->name : '--';
        });
    ...
    protected function getColumns()
    {
        $data = [
            ...
            __('CFA Stockist') => ['data' => 'name', 'name' => 'name', 'exportable' => false, 'title' => __('CFA Stockist')],
            __('app.customers') => ['data' => 'stockist_name', 'name' => 'stockist_name', 'visible' => false, 'title' => __('app.customers')],
