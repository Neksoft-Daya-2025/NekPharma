...
class ExpensesDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('action', function ($row) {
            ...
            $isPendingOrApproved = in_array($row->status, ['pending', 'approved']);
            $canDelete = false;
            
            if ($isPendingOrApproved) {
                // Only admin can delete pending/approved expenses
                $canDelete = $this->deleteExpensePermission == 'all';
            } else {
                // Normal delete permission check for other expenses
                $canDelete = $this->deleteExpensePermission == 'all' || ($this->deleteExpensePermission == 'added' && user()->id == $row->added_by);
            }
