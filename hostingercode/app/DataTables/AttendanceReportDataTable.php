...
class AttendanceReportDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->addColumn('report_employee_id', function ($row) {
                ...
                return $ed->employee_id ?? '-';
            })
            ->addColumn('report_designation', function ($row) {
                $ed = $row->employeeDetail;
                return $ed && $ed->designation ? $ed->designation->name : '-';
            })
            ->addColumn('report_department', function ($row) {
                $ed = $row->employeeDetail;
                return $ed && $ed->department ? $ed->department->team_name : '-';
            })
            ->addColumn('report_joining_date', function ($row) {
                $ed = $row->employeeDetail;
                return $ed && $ed->joining_date ? \Carbon\Carbon::parse($ed->joining_date)->translatedFormat($this->company->date_format) : '-';
            })
    ...
    {
        $request = $this->request();
        $model = User::with('role', 'roles', 'employeeDetail', 'employeeDetail.designation', 'employeeDetail.department', 'employeeDetail.headquarter', 'session')
    ...
    protected function getColumns()
    {
        return [
            ...
            __('app.designation') => ['data' => 'report_designation', 'name' => 'report_designation', 'title' => __('app.designation')],
            __('app.department') => ['data' => 'report_department', 'name' => 'report_department', 'title' => __('app.department')],
            __('app.dateOfJoining') => ['data' => 'report_joining_date', 'name' => 'report_joining_date', 'title' => __('app.dateOfJoining')],
            __('app.hq') => ['data' => 'report_headquarter', 'name' => 'report_headquarter', 'title' => __('app.hq')],
