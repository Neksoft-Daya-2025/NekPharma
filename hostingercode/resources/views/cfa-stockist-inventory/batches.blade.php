    ...
    <div class="content-wrapper">
        <div class="d-flex justify-content-between action-bar mb-2 mb-lg-0 mb-md-0">
            <div class="d-flex align-items-center">
                <x-forms.link-secondary :link="route('cfa-stockist-inventory.index')" class="mr-3" icon="arrow-left">
                    @lang('app.back')
                    ...
                    <tbody>
                        @foreach($batches as $batch)
                            <tr>
                                ...
                                <td>
                                    @if($batch->invoice)
                                        <a href="{{ route('cfa-stockist-invoices.show', $batch->invoice_id) }}" class="text-dark">{{ $batch->invoice->invoice_number }}</a>
                                    @else
                                        -
                                    @endif
                                </td>
                                <td>
                                    <div class="dropdown dropup">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="batchMenu-{{ $batch->id }}">
                                            <a href="{{ route('cfa-stockist-inventory.batches.edit', $batch->id) }}" class="dropdown-item openRightModal">
        ...
        $('body').on('click', '.delete-batch-btn', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('cfa-stockist-inventory.batches.destroy', ':id') }}".replace(':id', id);
