        ...
        <div class="d-block d-lg-flex d-md-flex justify-content-between">
            <div id="table-actions" class="flex-grow-1 align-items-center mb-2 mb-lg-0 mb-md-0">
                @if ($addInvoicesPermission == 'all' || $addInvoicesPermission == 'added')
                    <x-forms.link-primary :link="route('cfa-distributor-invoices.create')" class="mr-3 float-left mb-2 mb-lg-0 mb-md-0"
                        icon="plus">
                        @lang('app.add') CFA/Distributor Invoice
                    </x-forms.link-primary>
                    <x-forms.link-secondary :link="route('cfa-distributor-invoices.create', ['type' => 'igst'])" class="mr-3 float-left mb-2 mb-lg-0 mb-md-0"
                        icon="plus">
                        Add IGST Invoice
...
@push('scripts')
    ...
    <script>
        ...
        $('#cfa-distributor-invoices-table').on('preXhr.dt', function(e, settings, data) {
        ...
        const showTable = () => {
            window.LaravelDataTables["cfa-distributor-invoices-table"].draw(true);
        }
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.easyAjax({
                            type: 'POST',
                            url: url,
                            container: '#cfa-distributor-invoices-table',
                            blockUI: true,
                            data: {
                                '_token': token,
                                'data_type' : dataType
        ...
        $(document).on('change', '.delivery-status-toggle', function() {
            var $toggle = $(this);
            var invoiceId = $toggle.data('invoice-id');
            var isChecked = $toggle.is(':checked');
            var deliveryStatus = isChecked ? 'received' : 'in_transit';
            ...
            $.easyAjax({
                url: "{{ route('cfa-distributor-invoices.update-delivery-status', ':id') }}".replace(':id', invoiceId),
                type: "PATCH",
                data: {
                ...
                success: function(response) {
                    if (response.status == 'success') {
                        ...
                        $badge.addClass('badge-' + response.status_class);
                        $badge.text(response.status_text);
                        
                        // Show success message
                        Swal.fire({
                            icon: 'success',
                            text: '@lang("messages.updateSuccess")',
                            toast: true,
                            position: 'top-end',
                            timer: 2000,
                            timerProgressBar: true,
                ...
                error: function(xhr) {
                    ...
                    var errorMessage = 'Error updating delivery status.';
                ...
                complete: function() {
                    // Custom complete overrides easyAjax default — must unblock UI or loading never stops
                    $.easyUnblockUI('body');
        ...
        $(document).on('click', '.payment-status-badge', function(e) {
            ...
            var url = "{{ route('cfa-distributor-invoices.payment-modal') }}";
