...
<style>
    ...
    /* Sticky summary so total is always visible when scrolling */
    .summary-section {
        position: sticky;
        bottom: 0;
        z-index: 10;
    ...
    .items-empty-state .text-muted { font-size: 14px; }
    .add-product-card {
        border: 2px solid #667eea;
        background: linear-gradient(to bottom, #f8f9ff 0%, #f8f9fa 100%);
    }
    /* Item row: clear border and spacing so each line is easy to see */
    #sortable .item-row {
        border: 1px solid #dee2e6;
        border-radius: 8px;
        margin-bottom: 12px;
    ...
    #sortable .item-row .c-inv-desc-table {
        min-width: 1100px;
    }
    #sortable .item-row .c-inv-desc-table table {
        margin-bottom: 0;
        width: 100%;
    }
    #sortable .item-row .c-inv-desc-table td {
        vertical-align: middle;
        padding: 6px 8px;
        white-space: nowrap;
    }
    #sortable .item-row .c-inv-desc-table td:first-child {
        white-space: normal;
        min-width: 120px;
    }
    #sortable .item-row .c-inv-desc-table input.form-control {
        min-width: 0;
    }
    #sortable .item-row .c-inv-desc-table .form-control {
        padding: 4px 6px;
        font-size: 12px;
    }
    /* Hide drag-handle column on create to save space; sort_order input stays in DOM for submit */
    #sortable .item-row > div:first-child {
        display: none;
    }
</style>
...
<div class="invoice-wrapper">
    ...
    <x-form class="c-inv-form" id="saveInvoiceForm">
        <input type="hidden" name="type" id="invoice_type" value="{{ request('type', '') }}">
                ...
                <div class="col-md-3">
                    ...
                    <div class="d-flex align-items-center mt-2">
                        <label class="mr-3 mb-0">
                            <input type="radio" name="delivery_status" value="in_transit" checked class="mr-1">
                            <span>In Transit</span>
                        </label>
                        <label class="mb-0">
                            <input type="radio" name="delivery_status" value="received" class="mr-1">
                            <span>Received</span>
                        </label>
                    </div>
                </div>
        ...
        <div class="client-section">
            <div class="section-title"><span class="step-badge">1</span> Bill To</div>
            <div class="row">
                <div class="col-md-6">
        ...
        <div class="items-section">
            <div class="section-title mb-3"><span class="step-badge">2</span> Add products to invoice</div>
            
            <!-- Add Product first so filling flow is: select product → add → see list -->
            <div class="dynamic-add-product-section add-product-card p-4 mb-4 rounded">
                <div class="row align-items-end">
                    <div class="col-md-4">
                        <x-forms.label fieldId="dynamic-add-products" fieldLabel="Product">
                        </x-forms.label>
                        <div class="select-others height-35 rounded">
                    ...
                    <div class="col-md-6">
                        <div id="dynamic-batch-selection-wrapper" style="display: none;">
                            <x-forms.label fieldId="dynamic-batch-buttons-container" fieldLabel="Select batch (click to add)">
                            </x-forms.label>
                            <div id="dynamic-batch-buttons-container" class="d-flex flex-wrap gap-2"></div>
                        </div>
                    </div>
                </div>
                <small class="text-muted mt-2 d-block">Select a product, set quantity, then click a batch to add the line. You can edit Scheme, MRP, PTS, PTR in the table below.</small>
            </div>

            <div class="d-flex justify-content-between align-items-center mb-2">
                <div class="section-title mb-0" style="border-bottom: none;">Invoice items</div>
            </div>
            <div id="items-empty-state" class="items-empty-state">
                <span class="text-muted">No items yet. Select a product above and click a batch to add lines.</span>
            </div>
            <div class="items-table-wrapper" id="items-table-wrapper" style="display: none;">
                <div id="sortable"></div>
            </div>
        </div>

        <div class="summary-section">
            <div class="section-title mb-2"><span class="step-badge">3</span> Summary</div>
...
// Store invoice type globally for use in AJAX calls (default cfa-distributor on this page so add_item hides Unit price)
var invoiceType = getUrlParameter('type') || 'cfa-distributor';
    ...
    $('.item-row').each(function(index) {
        ...
        if (pts > 0 && paidQty > 0) {
            ...
            if ($schemeDisplay.length && $schemePaidText.length && scheme && scheme.indexOf('+') !== -1 && freeQty > 0) {
                // Show total quantity with breakdown in brackets: "240 (219+21)"
                $schemePaidText.html(
                    '<span class="text-dark font-weight-bold">' + totalQty + ' (' + paidQty + '+' + freeQty + ')</span>'
                );
                $schemeDisplay.show();
...
$(document).on('input change', 'input[name="pts[]"], input[name="dis[]"], input[name="scheme[]"], select[name*="taxes"], #exchange_rate', function() {
    calculateCFAInvoiceTotal();
});
