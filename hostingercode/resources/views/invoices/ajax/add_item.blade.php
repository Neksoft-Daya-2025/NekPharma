        ...
        <table width="100%">
            <tbody>
                <tr class="text-dark-grey font-weight-bold f-14">
                    ...
                    <td width="3%" class="border-0" align="center">Exp</td>
                    ...
                    @endif
                    @if(!isset($invoiceType) || $invoiceType != 'cfa-distributor')
                ...
                <tr>
                    ...
                    <td class="border-bottom-0">
                        {{-- Scheme field is empty by default - user can add their own scheme --}}
                        <input type="text" class="form-control f-12 w-100 text-center scheme-input" name="scheme[]"
                            value="" placeholder="10+1 (10 paid + 1 free)" 
                            title="Enter scheme format: paid+free (e.g., 10+1 means 10 paid + 1 free). Leave empty if no scheme."
                    ...
                    <td class="border-bottom-0" style="width: 3%; min-width: 70px;">
                        @php
                            $expiryValue = '';
                        ...
                        @endphp
                        <input type="date" class="form-control f-9 border-0 w-100 text-center batch-exp-input" name="exp[]"
                            value="{{ $expiryValue }}" placeholder="Exp" readonly style="padding: 1px 2px; font-size: 9px;">
                    </td>
                    <td class="border-bottom-0">
                        <input type="number" class="form-control f-12 w-100 text-right batch-mrp-input" name="mrp[]"
                            value="{{ isset($purchaseEntry) ? ($purchaseEntry->mrp ?? '') : '' }}" placeholder="MRP" step="0.01"
                            style="border: 1px solid #ced4da; border-radius: 4px;">
                    </td>
                    <td class="border-bottom-0">
                        <input type="number" class="form-control f-12 w-100 text-right batch-pts-input" name="pts[]"
                            value="{{ isset($purchaseEntry) ? ($purchaseEntry->pts ?? '') : '' }}" placeholder="PTS" step="0.01"
                            style="border: 1px solid #ced4da; border-radius: 4px;">
                    </td>
                    <td class="border-bottom-0">
                        <input type="number" class="form-control f-12 w-100 text-right batch-ptr-input" name="ptr[]"
                            value="{{ isset($purchaseEntry) ? ($purchaseEntry->ptr ?? '') : '' }}" placeholder="PTR" step="0.01"
                            style="border: 1px solid #ced4da; border-radius: 4px;">
                    </td>
                    <td class="border-bottom-0">
                        @php
                        ...
                        <input type="number" step="0.01" min="0" max="100" class="form-control f-12 text-right batch-dis-input item-discount-input" name="dis[]"
                            value="{{ $disValue }}" placeholder="DIS %" data-item-id="{{ $items->id ?? '' }}"
                    ...
                    <td class="d-none p-0 border-0" style="width:0;line-height:0;">
                        <input type="hidden" class="cost_per_item" name="cost_per_item[]" value="{{ isset($purchaseEntry) ? ($purchaseEntry->pts ?? $items->price) : $items->price }}">
                    </td>
                    @endif
                    @php
                        ...
                        // First check invoice item taxes (for edit mode) - highest priority
                        ...
                        // If no taxes from invoice item, check purchase entry taxes
                        if (empty($selectedTaxIds) && isset($purchaseEntry) && $purchaseEntry->tax) {
                        ...
                        // If still no taxes, check product taxes (from $items->taxes which is set by controller)
                        if (empty($selectedTaxIds) && isset($items->taxes) && !empty($items->taxes)) {
                            if (is_array($items->taxes)) {
                                $selectedTaxIds = $items->taxes;
                            } elseif (is_string($items->taxes)) {
                        ...
                        $selectedTaxIds = is_array($selectedTaxIds) ? array_unique($selectedTaxIds) : [];
                        
                        // Get SGST and CGST separately from available taxes
                        if (isset($taxes) && is_iterable($taxes)) {
                    ...
                    @if($isIGST)
                        ...
                        <td class="border-bottom-0">
                            ...
                            <select name="taxes[][]" multiple="multiple" class="d-none tax-select-hidden">
                                @foreach ($taxes as $tax)
    ...
    <script>
        $(function() {
            ...
            function calculateItemAmount($row) {
                ...
                var $taxSelect = $row.find('.tax-select-hidden, select[name="taxes[][]"], select[name*="taxes"]');
                if ($taxSelect.length) {
            ...
            // Function to update tax display when taxes change - separate columns for SGST and CGST, or IGST for IGST invoices
            function updateTaxDisplay($row, taxIds) {
                var isIGST = getUrlParameter('type') === 'igst';
                var $sgstDisplay = $row.find('.sgst-display-wrapper');
                var $cgstDisplay = $row.find('.cgst-display-wrapper');
                var $igstDisplay = $row.find('.igst-display-wrapper');
                ...
                if (isIGST) {
                    // For IGST invoices: Calculate IGST = SGST + CGST
                    var igstRate = 0;
                    if (sgstRate !== null && cgstRate !== null) {
                        igstRate = sgstRate + cgstRate;
                    } else if (sgstRate !== null) {
                        igstRate = sgstRate * 2; // If only SGST, assume equal split
                    } else if (cgstRate !== null) {
                        igstRate = cgstRate * 2; // If only CGST, assume equal split
                    }
                    
                    // Update IGST display
                    if ($igstDisplay.length) {
                        if (igstRate > 0) {
                            $igstDisplay.html('<div class="f-12 text-dark-grey text-center">' + igstRate.toFixed(2) + '%</div>');
                        } else {
                            $igstDisplay.html('<div class="f-12 text-lightest text-center">--</div>');
                        }
                    }
                } else {
                    // For regular invoices: Update SGST and CGST separately
                    // Update SGST display
                    if ($sgstDisplay.length) {
                        if (sgstRate !== null) {
