        ...
        <div class="row px-lg-4 px-md-4 px-3 pt-3">
            ...
            <div class="col-md-4">
                @if (isset($isCFADistributor) && $isCFADistributor == true)
                    <!-- Show Stockist dropdown for CFA/Distributor -->
                    <div class="form-group">
                        <x-forms.label fieldId="stockist_id" :fieldLabel="__('Stockist')" fieldRequired="true">
                        </x-forms.label>
                        <div class="select-others height-35 rounded">
                            <select class="form-control select-picker" data-live-search="true" data-size="8"
                                ...
                                @foreach ($stockists as $stockist)
                                    <option value="{{ $stockist->id }}" 
                                        @selected($invoice->stockist_id == $stockist->id)>
                                        {{ $stockist->shopname }} - {{ $stockist->fullname }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
            ...
            <div class="col-md-4">
                <div class="form-group c-inv-select mb-lg-0 mb-md-0 mb-4">
                    ...
                    <p class="f-15" id="client_shipping_address">
                        @if ($invoice->stockist && $invoice->stockist->address)
                            {!! nl2br($invoice->stockist->address) !!}
                        @elseif ($invoice->client?->clientDetails?->shipping_address)
                ...
                <div class="col-md-3">
                    ...
                    <x-forms.input-group>
                        ...
                        @if (module_enabled('Purchase'))
                            @foreach ($products as $purchaseEntry)
                                @php
                                    $product = $purchaseEntry->product;
                                @endphp
                                @if($product)
                                    <option data-content="{{ $product->name }} @if($purchaseEntry->batch) - Batch: {{ $purchaseEntry->batch }} @endif" 
                                        value="{{ $purchaseEntry->id }}" 
                                        data-purchase-entry-id="{{ $purchaseEntry->id }}">
                                        {{ $product->name }}
                                        @if($purchaseEntry->batch)
...
<script>
            ...
            success: function(response) {
                if (response.status == 'success') {
                    if (response.data !== null) {
                        // Update billing address with stockist address
                        if (response.data.address) {
                            $('#client_billing_address').html(nl2br(response.data.address)).removeClass('d-none');
                        ...
                        if (response.data.address) {
                            $('#client_shipping_address').html(nl2br(response.data.address));
                        } else {
                            var addShippingLink =
    ...
    $(document).ready(function() {
        // Handle stockist selection change
        ...
        function addProduct(id) {
            ...
            // Check if this is an invoice item (has data-invoice-item-id attribute)
