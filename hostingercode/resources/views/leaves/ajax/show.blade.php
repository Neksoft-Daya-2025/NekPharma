        ...
        <div class="col-sm-12">
            <div class="card bg-white border-0 b-shadow-4">
                                ...
                                @if ($leave->status == 'pending' || $leave->status == 'manager_approved')
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0"
                                        ...
                                        @php
                                            ...
                                            $isReportee = $reportingTo && optional($leave->user->employeeDetail)->reporting_to == user()->id;
                                        @endphp
                                            @if ($leave->status == 'pending' && $reportingTo && $leave->user_id != user()->id && !($reportingPermission == 'cannot-approve'))
                                                @if(!$rejectActionAdded && ($isReportee || $approveRejectPermission == 'all') && $leave->manager_status_permission == '')
                                                ...
                                                @if (!$approvedActionAdded && $isReportee && $reportingPermission == 'approved' && $leave->manager_status_permission == '')
                                                ...
                                                @elseif(!$approvedActionAdded && $isReportee && $reportingPermission == 'pre-approve' && !$leave->manager_status_permission)
                                            ...
                                            @if ($leave->status == 'manager_approved' && $approveRejectPermission == 'all')
                                            ...
                                            @if ($leave->status == 'pending' && ($editLeavePermission == 'all'
                ...
                <div class="card-body">
                    ...
                    @if (!is_null($leave->manager_approved_by) && $leave->managerApprovedBy)
                        <div class="col-12 px-0 pb-3 d-lg-flex d-md-flex d-block">
                            <p class="mb-0 text-lightest f-14 w-30 d-inline-block ">
                                @lang('modules.leaves.managerApprovedBy')</p>
                            <p class="mb-0 text-dark-grey f-14">
                                <x-employee :user="$leave->managerApprovedBy" />
                            </p>
                        </div>
                    @endif

                    @if (!is_null($leave->manager_approved_at))
                        <x-cards.data-row :label="__('modules.leaves.managerApprovedAt')" :value="$leave->manager_approved_at->timezone(company()->timezone)->translatedFormat(company()->date_format .' ' . company()->time_format)" />
                    @endif
