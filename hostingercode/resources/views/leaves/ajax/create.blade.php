                        ...
                        @else
                            <x-forms.select fieldId="user_id" :fieldLabel="__('modules.messages.chooseMember')" fieldName="user_id" search="true"
                                ...
                                @foreach ($employees as $employee)
                                    <x-user-option :user="$employee" :employeeSelect="true" :selected="request()->has('default_assign') &&
                                ...
                                @if (isset($leaveTypes))
                                    @foreach ($leaveTypes as $leaveType)
                                        @if($leaveType->deleted_at == null)
                                            <option value="{{ $leaveType->id }}">{{ $leaveType->type_name }} ({{ (int) round((float) $leaveType->no_of_leaves) }})
                                            </option>
                                        @endif
                                    @endforeach
                                @endif
                                ...
                                @if (isset($leaveQuotas))
                                    @foreach ($leaveQuotas as $leaveQuota)
                                        @if($leaveQuota->leaveType && $leaveQuota->leaveType->leaveTypeCondition($leaveQuota->leaveType, $defaultAssign)
                                            ...
                                            <option value="{{ $leaveQuota->leaveType->id }}">{{ $leaveQuota->leaveType->type_name }} ({{ rtrim(rtrim(number_format((float) $leaveQuota->leaves_remaining, 1, '.', ''), '0'), '.') }})</option>
                                        @endif
                                    @endforeach
                                @endif
                            </select>
