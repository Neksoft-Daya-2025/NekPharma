        ...
        <x-form id="save-data-form" method="PUT">
            <div class="add-client bg-white rounded">
                    ...
                    <div class="col-md-3">
                        ...
                        <x-forms.input-group>
                            <select class="form-control select-picker" name="category_id" id="category_id"
                                data-live-search="true" data-html="true">
                                <option value="">--</option>
                ...
                <div class="row p-20">
                    ...
                    <div class="col-lg-3 col-md-6">
                        <x-forms.text class="mb-3 mt-3 mt-lg-0 mt-md-0" fieldId="cfa_distributor_code"
                            fieldLabel="CFA/Distributor Code (User ID)" fieldName="cfa_distributor_code"
                            :fieldValue="$client->clientDetails->cfa_distributor_code ?? ''"
                            fieldPlaceholder="e.g. CFA-001">
                        </x-forms.text>
                        <small class="form-text text-muted">Optional display ID for CFA/Distributor</small>
                    </div>
                    <div class="col-lg-3 col-md-6">
                    ...
                    <div class="col-lg-3 col-md-6">
                        <x-forms.text class="mb-3 mt-3 mt-lg-0 mt-md-0" fieldId="dl_number"
                            fieldLabel="DL Number" :fieldValue="$client->clientDetails->dl_number ?? ''"
                            fieldName="dl_number" fieldPlaceholder="Enter DL Number"></x-forms.text>
                    </div>
                    <div class="col-lg-3 col-md-6">
                        <x-forms.label class="my-3" fieldId="area_ids" fieldLabel="Area Allotment">
                        </x-forms.label>
                        <x-forms.input-group>
                            <select class="form-control select-picker" name="area_ids[]" id="area_ids"
                                data-live-search="true" multiple="true" data-html="true">
                                @foreach ($areas as $area)
                                    <option value="{{ $area->id }}" 
                                        @selected($client->clientDetails && $client->clientDetails->areas->contains($area->id))>
                                        {{ $area->name }}
                                    </option>
                                @endforeach
                            </select>
                        </x-forms.input-group>
                        <small class="form-text text-muted">Select multiple areas for CFA and Distributor</small>
                    </div>
                    <div class="col-lg-3 col-md-6">
                        <x-forms.label class="my-3" fieldId="headquarter_ids" fieldLabel="Assigned HQ">
                        </x-forms.label>
                        <x-forms.input-group>
                            <select class="form-control select-picker" name="headquarter_ids[]" id="headquarter_ids"
                                data-live-search="true" multiple="true" data-html="true">
                                @foreach ($headquarters as $hq)
                                    <option value="{{ $hq->id }}"
                                        @selected($client->clientDetails && $client->clientDetails->headquarters && $client->clientDetails->headquarters->contains('id', $hq->id))>
                                        {{ $hq->name }}
                                    </option>
                                @endforeach
                            </select>
                        </x-forms.input-group>
                        <small class="form-text text-muted">Select HQs for CFA and Distributor</small>
                    </div>
                    ...
                    <div class="col-lg-4 col-md-6">
                        <x-forms.text fieldId="bank_account_name" fieldLabel="Account Name"
                            fieldName="bank_account_name" fieldPlaceholder="Enter Account Name"
                            :fieldValue="$client->clientDetails->bank_account_name ?? ''">
                        </x-forms.text>
                    </div>
                    <div class="col-lg-4 col-md-6">
                        <x-forms.text fieldId="bank_account_number" fieldLabel="Account Number"
                            fieldName="bank_account_number" fieldPlaceholder="Enter Account Number"
                            :fieldValue="$client->clientDetails->bank_account_number ?? ''">
                        </x-forms.text>
                    </div>
                    <div class="col-lg-4 col-md-6">
                        <x-forms.text fieldId="bank_ifsc_code" fieldLabel="IFSC Code"
                            fieldName="bank_ifsc_code" fieldPlaceholder="Enter IFSC Code"
                            :fieldValue="$client->clientDetails->bank_ifsc_code ?? ''">
                        </x-forms.text>
                    </div>
                    <div class="col-lg-3 col-md-6">
                            ...
                            <x-forms.select fieldId="added_by" :fieldLabel="__('app.added').' '.__('app.by')"
                                ...
                                @foreach ($employees as $item)
                                    @if($item->status == 'active' || $client->clientDetails->added_by == $item->id)
                                        <x-user-option :user="$item" :employeeSelect="true" :selected="$client->clientDetails->added_by == $item->id" />
                                    @endif
                                @endforeach
                            </x-forms.select>
                        </div>
