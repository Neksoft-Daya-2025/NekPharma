...
@section('filter-section')
    <x-filters.filter-box>
        ...
        <div class="select-box d-flex py-2 px-lg-2 px-md-2 px-0 border-right-grey border-right-grey-sm-0">
            <p class="mb-0 pr-2 f-14 text-dark-grey d-flex align-items-center">@lang('app.cfaStockist')</p>
            <div class="select-status">
                <select class="form-control select-picker" id="partyID" data-live-search="true" data-size="8">
                    <option value="all">@lang('app.all')</option>
                    @foreach ($cfaStockists as $st)
                        <option value="{{ $st->id }}" {{ $partyId == $st->id ? 'selected' : '' }}>{{ $st->cfa_stockist_id ?? '' }} {{ $st->shopname }} {{ $st->fullname ? ' - ' . $st->fullname : '' }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div id="ledger-table-wrap" class="d-flex flex-column w-tables rounded mt-3 bg-white table-responsive">
            <table class="table table-hover border-0 w-100" id="ledger-table">
                ...
                <tbody id="ledger-tbody">
                    <tr><td colspan="5" class="text-center text-muted">Select a stockist and click Apply</td></tr>
                </tbody>
            </table>
        </div>
    </div>
...
<script>
    $(function () {
        ...
        $('#apply-ledger-filters').on('click', function () {
            ...
            $.ajax({
                url: '{{ route("cfa-stockist-ledger.data") }}',
                type: 'GET',
