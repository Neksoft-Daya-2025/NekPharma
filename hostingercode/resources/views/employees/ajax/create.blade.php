...
<div class="row">
    <div class="col-sm-12">
        <x-form id="save-employee-data-form">
            <!-- Bypass email sending on employee creation -->
                        ...
                        <div class="row">
                            <div class="col-md-3">
                                <x-forms.text fieldId="employee_id" :fieldLabel="__('modules.employees.employeeId')"
                                    fieldName="employee_id" :fieldValue="((!$checkifExistEmployeeId) ? $generatedEmployeeID : '')" fieldRequired="true"
                                    :fieldPlaceholder="__('modules.employees.employeeIdInfo')" :popover="__('modules.employees.employeeIdHelp')" readonly="true" class="bg-light">
                                </x-forms.text>
                            </div>
                            ...
                            <div class="col-lg-4 col-md-6">
                                ...
                                <x-forms.input-group>
                                    <input type="text" name="password" id="password"
                                        class="form-control height-35 f-14 bg-light" readonly>
                                    <x-slot name="append">
  ...
  <!--=================================================== Sonu ===================================================                          -->
                            <!-- AREA MULTI SELECT FIELD -->
                            <div class="col-lg-4 col-md-6 d-none" id="areaDiv">
...
<!--==================================================================================================================================-->
                            ...
                            <div class="col-lg-4 col-md-6" id="headquarterDiv">
                                ...
                                <x-forms.input-group>
                                    ...
                                    <x-slot name="append">
                                        <a href="{{ route('pharma-areas.headquarters') }}" target="_blank" 
                                           class="btn btn-outline-secondary border-grey" 
                                           data-toggle="tooltip" title="Add HeadQuarter">
                        ...
                        <x-forms.select fieldId="country" :fieldLabel="__('app.country')" fieldName="country"
                            ...
                            @foreach ($countries as $item)
                                <option data-tokens="{{ $item->iso3 }}" data-phonecode = "{{$item->phonecode}}" data-iso="{{ $item->iso }}"
                                    ...
                                    value="{{ $item->id }}" {{ $item->nicename == 'India' ? 'selected' : '' }}>{{ $item->nicename }}</option>
                            @endforeach
                        </x-forms.select>
                    </div>
                    <div class="col-lg-3 col-md-6">
                            ...
                            <x-forms.select fieldId="country_phonecode" fieldName="country_phonecode"
                                ...
                                @foreach ($countries as $item)
                                    <option data-tokens="{{ $item->name }}" data-country-iso="{{ $item->iso }}"
                                            ...
                                            value="{{ $item->phonecode }}" {{ $item->nicename == 'India' ? 'selected' : '' }}>{{ $item->phonecode }}
                                    </option>
                                @endforeach
                            </x-forms.select>
                    ...
                    <div class="col-lg-3 col-md-6">
                        <x-forms.select fieldId="employment_type" :fieldLabel="__('modules.employees.employmentType')"
                            ...
                            @foreach($employmentTypes as $empType)
                                <option value="{{ $empType->slug }}" data-requires-end-date="{{ $empType->requires_end_date ? '1' : '0' }}">
                                    {{ $empType->name }}
                                </option>
                            @endforeach
                        </x-forms.select>
...
<script>
    $(document).ready(function() {
        ...
        $('#employment_type').change(function(){
            ...
            var requiresEndDate = selectedOption.data('requires-end-date');
            
            // Show/hide end date fields based on employment type requirement
            if(requiresEndDate == '1') {
                // Show appropriate end date field based on slug
                if(value == 'on_contract') {
                    ...
                    // For other types that require end date, show contract date field
                    $('.contract-date').removeClass('d-none');
                    $('.internship-date').addClass('d-none');
                    $('#internship_end_date').val('');
                }
        ...
        // Also allow manual regeneration
...
$(document).ready(function () {
    ...
    // Run when designation changes
    $("#employee_designation").on("change", toggleAreaRegion);
