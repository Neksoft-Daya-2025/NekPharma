<div class="modal-header">
    <h5 class="modal-title">@lang('app.edit') Doctor</h5>
    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
</div>

@php
    $headquarterOptions = $headquarters ?? collect();
    $defaultHeadquarterId = $defaultHeadquarterId ?? $doctor->headquarter_id;
@endphp

<div class="modal-body">
    <x-form id="update-doctor-form" method="PUT" class="ajax-form">
        <div class="row">
            <div class="col-md-6">
                <x-forms.text fieldId="fullname" :fieldLabel="__('app.name')" fieldName="fullname"
                    fieldRequired="true" :fieldPlaceholder="__('placeholders.name')" :fieldValue="$doctor->fullname">
                </x-forms.text>
            </div>

            <div class="col-md-6">
                <x-forms.email fieldId="email" :fieldLabel="__('app.email')" fieldName="email"
                    :fieldPlaceholder="__('placeholders.email')" :fieldValue="$doctor->email">
                </x-forms.email>
            </div>

            <div class="col-md-6">
                <x-forms.text fieldId="qualification" fieldLabel="Qualification" fieldName="qualification"
                    fieldPlaceholder="e.g., MBBS, MD, MS" :fieldValue="$doctor->qualification">
                </x-forms.text>
            </div>

            <div class="col-md-6">
                <x-forms.text fieldId="speciality" fieldLabel="Speciality" fieldName="speciality"
                    fieldPlaceholder="e.g., Cardiologist, Pediatrician" :fieldValue="$doctor->speciality">
                </x-forms.text>
            </div>

            <div class="col-md-6">
                <x-forms.tel fieldId="mobile" :fieldLabel="__('app.mobile')" fieldName="mobile"
                    :fieldPlaceholder="__('placeholders.mobile')" :fieldValue="$doctor->mobile">
                </x-forms.tel>
            </div>
            ...
            <div class="col-md-6">
                ...
                <select class="form-control select-picker" name="headquarter_id" id="headquarter_id" data-live-search="true" required>
                    ...
                    @foreach($headquarterOptions as $hq)
                        <option value="{{ $hq->id }}" @if($doctor->headquarter_id == $hq->id) selected @endif>
                            {{ $hq->name }}
                ...
                <div class="d-flex align-items-center flex-wrap">
                    <div class="custom-control custom-radio mr-4">
                        <input type="radio" class="custom-control-input" name="station_type" id="type_headquarter" value="headquarter"
                            @if(!$doctor->exstation_id && !$doctor->outstation_id) checked @endif>
                    ...
                    <div class="custom-control custom-radio mr-4">
                        <input type="radio" class="custom-control-input" name="station_type" id="type_exstation" value="exstation"
                            @if($doctor->exstation_id) checked @endif>
                    ...
                    <div class="custom-control custom-radio">
                        <input type="radio" class="custom-control-input" name="station_type" id="type_outstation" value="outstation"
                            @if($doctor->outstation_id) checked @endif>
            ...
            <div class="col-md-6" id="exstation_field" style="display: {{ $doctor->exstation_id ? 'block' : 'none' }};">
            ...
            <div class="col-md-6" id="outstation_field" style="display: {{ $doctor->outstation_id ? 'block' : 'none' }};">
            ...
            <div class="col-md-6">
                <x-forms.datepicker fieldId="dob" fieldLabel="Date of Birth" fieldName="dob"
                    :fieldPlaceholder="__('placeholders.date')" :fieldValue="$doctor->dob ? $doctor->dob->format(company()->date_format) : ''" custom="true">
                </x-forms.datepicker>
            </div>

            <div class="col-md-6">
                <x-forms.datepicker fieldId="dom" fieldLabel="Date of Marriage" fieldName="dom"
                    :fieldPlaceholder="__('placeholders.date')" :fieldValue="$doctor->dom ? $doctor->dom->format(company()->date_format) : ''" custom="true">
                </x-forms.datepicker>
            </div>
            ...
            <div class="col-md-6">
                <x-forms.select fieldId="gender" :fieldLabel="__('modules.employees.gender')" fieldName="gender">
                    <option value="">--</option>
                    <option value="male" @if($doctor->gender == 'male') selected @endif>@lang('app.male')</option>
                    <option value="female" @if($doctor->gender == 'female') selected @endif>@lang('app.female')</option>
                    <option value="other" @if($doctor->gender == 'other') selected @endif>@lang('app.others')</option>
                </x-forms.select>
            </div>
                ...
                <x-forms.input-group>
                    <select class="form-control select-picker" name="doctor_type" id="doctor_type" data-live-search="true">
                        ...
                        @foreach($doctorTypes ?? ['VIP', 'CORE'] as $type)
                            <option value="{{ $type }}" @if($doctor->doctor_type == $type) selected @endif>{{ $type }}</option>
                        @endforeach
                    </select>
                    <x-slot name="append">
            ...
            <div class="col-md-12">
                <x-forms.file allowedFileExtensions="png jpg jpeg" class="mr-0 mr-lg-2 mr-md-2"
                    :fieldLabel="__('Doctor Photo')" fieldName="doctor_pic" fieldId="doctor_pic" 
                    fieldHeight="200"
                    :fieldValue="$doctor->doctor_pic ? asset_url('doctor/'.$doctor->doctor_pic) : ''">
                </x-forms.file>
            </div>

            <div class="col-md-12">
                <x-forms.textarea class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('app.address')"
                    fieldName="address" fieldId="address" :fieldPlaceholder="__('placeholders.address')" :fieldValue="$doctor->address" fieldRequired="true">
                </x-forms.textarea>
            </div>

            <div class="col-md-6">
                <x-forms.text fieldId="msl_number" fieldLabel="MSL Number" fieldName="msl_number"
                    fieldPlaceholder="Enter MSL Number" :fieldValue="$doctor->msl_number">
                </x-forms.text>
            </div>

            <div class="col-md-3">
                <x-forms.number fieldId="latitude" fieldLabel="Latitude (GPS)" fieldName="latitude"
                    fieldPlaceholder="e.g. 28.6139" :fieldValue="$doctor->latitude ?? null">
                </x-forms.number>
            </div>
            <div class="col-md-3">
                <x-forms.number fieldId="longitude" fieldLabel="Longitude (GPS)" fieldName="longitude"
                    fieldPlaceholder="e.g. 77.2090" :fieldValue="$doctor->longitude ?? null">
                </x-forms.number>
            </div>
            ...
            <div class="col-md-12">
                ...
                <select class="form-control select-picker" name="products[]" id="products" multiple data-live-search="true" data-actions-box="true">
                    @foreach($products ?? [] as $product)
                        <option value="{{ $product->id }}" data-product-name="{{ $product->name }}" @if(in_array($product->id, $selectedProducts ?? [])) selected @endif>{{ $product->name }}</option>
                    @endforeach
                </select>
...
<div class="modal-footer">
    ...
    <x-forms.button-primary id="update-doctor" icon="check">@lang('app.update')</x-forms.button-primary>
</div>
...
<script>
    $(document).ready(function() {
        ...
        const existingHeadquarterId = @json($doctor->headquarter_id);
        const existingStationType = @json($doctor->exstation_id ? 'exstation' : ($doctor->outstation_id ? 'outstation' : 'headquarter'));
        const existingExstationId = @json($doctor->exstation_id);
        const existingOutstationId = @json($doctor->outstation_id);
        ...
        $('#update-doctor').click(function() {
            $.easyAjax({
                ...
                container: '#update-doctor-form',
                type: "POST",
                disableButton: true,
                blockUI: true,
                buttonSelector: "#update-doctor",
                file: true,
                data: $('#update-doctor-form').serialize(),
