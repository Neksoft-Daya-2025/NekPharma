...
@section('content')
    ...
    <div class="px-4 py-2 border-top-0 emp-dashboard">
        ...
        <!-- Tour Plan – Approved (3.2.4: shown on employee dashboard after approval) -->
        @if(isset($approvedTourPlanSummary) && $approvedTourPlanSummary && ($approvedTourPlanSummary['has_approved'] ?? false))
            <div class="row">
                <div class="col-md-12">
                    <a href="{{ route('tours.status') }}" class="text-dark">
                        <x-alert type="success" icon="check-circle">
                            <strong>@lang('modules.tours.tourPlan')</strong> – @lang('app.approved')
                            @if(!empty($approvedTourPlanSummary['latest_approved_month_label']))
                                <span class="d-inline-block ml-1">({{ $approvedTourPlanSummary['latest_approved_month_label'] }})</span>
                            @endif
                            <span class="d-inline-block ml-1">→ @lang('app.view') @lang('modules.tours.tourPlan')</span>
                        </x-alert>
                    </a>
                </div>
            </div>
