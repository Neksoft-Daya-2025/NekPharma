...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        // Auto-map columns if all required columns are matched
        let allRequiredMatched = true;
        let requiredColumns = jsColumnArray.filter(col => col.required === "Yes");
        requiredColumns.forEach(function(requiredCol) {
            if (!contains(jsMatchedColumnArray, requiredCol.id)) {
        ...
        if (allRequiredMatched && jsMatchedColumnArray.length > 0) {
            ...
            // Only show column box if there are unmatched required columns
            let unmatchedRequired = requiredColumns.filter(col => !contains(jsMatchedColumnArray, col.id));
