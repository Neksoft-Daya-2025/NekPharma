@php
    ...
    $dataEmployeeId = '';
    if (!empty($employeeSelect) && isset($user) && $user) {
        $dataEmployeeId = (string) (optional($user->employeeDetail)->employee_id
            ?? optional($user->employeeDetails)->employee_id
            ?? '');
    }
...
@endphp
    ...
    <option @selected($selected) @if (empty($employeeSelect)) data-content="{!! $content !!}" @endif value="{{ $userID ?? $user->id }}"
        @if (!empty($employeeSelect)) data-employee-id="{{ e($dataEmployeeId) }}" @endif
