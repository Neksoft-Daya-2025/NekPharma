    ...
    <table class="header-main-table">
        <tr>
            ...
            <td class="header-left">
                ...
                @endif
                <div class="company-name">{{ $cfaDistributorDetails->company_name ?? $cfaDistributor->name }}</div>
                <div class="cfa-name">CFA/Distributor</div>
            ...
            @foreach ($displayItems as $index => $item)
                ...
                @php
                    ...
                    if (empty($productName) || trim($productName) === '' || strtolower(trim($productName)) === 'cc') {
                        ...
                        // Try to get product name from CFAStockistStock product (priority 3)
                        elseif ($cfaStockistStock && $cfaStockistStock->product && !empty($cfaStockistStock->product->name)) {
                            $productName = $cfaStockistStock->product->name;
                        }
                        // Try to get product name from CFADistributorStock product (priority 4)
                        elseif ($cfaDistributorStock && $cfaDistributorStock->product && !empty($cfaDistributorStock->product->name)) {
                            $productName = $cfaDistributorStock->product->name;
                        }
