    ...
    <div class="content-wrapper">
            ...
            <div class="btn-group">
                @if (!empty($canEditCfaStockistInvoice) || $editInvoicePermission == 'all' || ($editInvoicePermission == 'added' && ($invoice->added_by == user()->id)))
                    @php
                        ...
                        if (!$isIGST && isset($invoice->invoice_type) && $invoice->invoice_type === 'igst') {
                            $isIGST = true;
                        }
                    @endphp
                    <a href="{{ route('cfa-stockist-invoices.edit', $invoice->id) }}{{ $isIGST ? '?type=igst' : '' }}" class="btn btn-secondary">
        ...
        <div class="card border-0 invoice">
            <div class="card-body" style="display: flex; justify-content: center; align-items: flex-start; padding: 20px;">
                @include('invoices.cfa-stockist.pharma-invoice')
            </div>
        </div>
    </div>
    <!-- CONTENT WRAPPER END -->
