@php
    ...
    // Header column: Invoiced by = CFA Distributor (invoice client)
    $invByName = '';
    if ($cfaDistributor) {
        if ($cfaDistributorDetails && trim((string) ($cfaDistributorDetails->company_name ?? '')) !== '') {
            $invByName = trim((string) $cfaDistributorDetails->company_name);
        } else {
            $invByName = trim((string) ($cfaDistributor->name ?? ''));
        }
    }
    $invByAddr = $cfaDistributorDetails && !empty($cfaDistributorDetails->address)
        ? trim((string) $cfaDistributorDetails->address)
        : '';
    $invByGst = $cfaDistributorDetails && !empty($cfaDistributorDetails->gst_number)
        ? trim((string) $cfaDistributorDetails->gst_number)
        : '';
    $invByStateCode = strlen($invByGst) >= 2 ? substr($invByGst, 0, 2) : '';
    $invByDl = $cfaDistributorDetails && !empty($cfaDistributorDetails->dl_number)
        ? trim((string) $cfaDistributorDetails->dl_number)
        : '';
@endphp
...
<div class="invoice-container pharma-invoice-body">
    @php
        ...
        $phiDlStockist = $stk && !empty($stk->dl_number) ? trim((string) $stk->dl_number) : '';
    @endphp
        ...
        <table>
            <tr>
                ...
                <td style="width:20%;vertical-align:top;">
                    <div class="phi-label" style="margin-bottom:4px;">INVOICED BY:</div>
                    @if($invByName !== '')
                        <div class="phi-kv">{{ $invByName }}</div>
                    @endif
                    @if($invByAddr !== '')
                        <div class="phi-kv">{{ $invByAddr }}</div>
                    @endif
                    @if($invByGst !== '')
                        <div class="phi-kv"><span class="phi-label">GSTIN:</span> {{ $invByGst }}</div>
                    @endif
                    @if($invByStateCode !== '')
                        <div class="phi-kv"><span class="phi-label">STATE CODE:</span> {{ $invByStateCode }}</div>
                    @endif
                    @if($invByDl !== '')
                        <div class="phi-kv"><span class="phi-label">DL:</span> {{ $invByDl }}</div>
                    @endif
                    @if($invByName === '' && $invByAddr === '' && $invByGst === '' && $invByDl === '')
                        <div class="phi-kv">—</div>
                    @endif
                </td>
        ...
        <table>
            <tr>
                <td style="width:50%;vertical-align:top;">
                    ...
                    @endif
                    @if ($phiBillPhone !== '')
                        <div class="phi-kv"><span class="phi-label">PHONE:</span> {{ $phiBillPhone }}</div>
                    @endif
                    @if ($phiCustGst !== '')
                    ...
                    @endif
                    @if ($phiDlStockist !== '')
                        <div class="phi-kv"><span class="phi-label">DL:</span> {{ $phiDlStockist }}</div>
                    @endif
                </td>
                <td style="width:50%;vertical-align:top;">
