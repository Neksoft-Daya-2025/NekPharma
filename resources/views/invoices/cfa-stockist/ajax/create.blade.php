...
<div class="invoice-wrapper">
    <div class="invoice-header">
        ...
        <div class="subtitle">@if(!empty($editingInvoice))@lang('app.edit') @else@lang('app.create') @endif — @lang('app.invoice')</div>
    </div>
            ...
            <div class="row">
                ...
                <div class="col-md-2">
                    ...
                    <input type="text" id="invoice_date" name="issue_date"
                        ...
                        value="{{ $cfaInvoiceIssueDate ?? now(company()->timezone)->format(company()->date_format) }}">
                </div>
                ...
                <div class="col-md-2">
                    ...
                    <input type="text" id="due_date" name="due_date"
                        class="form-control height-35 f-15"
                        placeholder="@lang('placeholders.date')"
                        value="{{ $cfaInvoiceDueDate ?? now(company()->timezone)->addDays($invoiceSetting->due_after)->format(company()->date_format) }}">
                </div>
                            ...
                            <select class="form-control select-picker" name="cfa_stockist_id" id="cfa_stockist_id" data-live-search="true">
                                ...
                                @foreach ($cfaStockists as $stockist)
                                    <option value="{{ $stockist->id }}"
                                        @if(!empty($editingCfaStockistId) && (int)$editingCfaStockistId === (int)$stockist->id) selected @endif>
...
<script>
window.cfaEditStockistId = @json((int) ($editingCfaStockistId ?? 0));
var addedBatches = {};
...
$(document).ready(function() {
    ...
    $('#save-form').click(function() {
        ...
        var url = @json(!empty($editingInvoice) ? route('cfa-stockist-invoices.update', $editingInvoice->id) : route('cfa-stockist-invoices.store'));
        var isEditMode = @json(!empty($editingInvoice));
        // Disabled fields are omitted from serializeArray(); ensure selects submit
        $('#cfa_stockist_id').prop('disabled', false);
        $('#cfa_distributor_id').prop('disabled', false);
        ...
        // Bootstrap-select / disabled fixes: always take live value
        var cfaStockistVal = $('#cfa_stockist_id').val();
        if (cfaStockistVal) {
            mappedData.cfa_stockist_id = cfaStockistVal;
        }
...
$(document).ready(function() {
    calculateCFAInvoiceTotal();
    @if(!empty($editingLineItems) && count($editingLineItems))
    var __cfaEditLines = @json($editingLineItems);
    if (__cfaEditLines && __cfaEditLines.length) {
        addedBatches = {};
        var __idx = 0;
        function __loadNextEditLine() {
            if (__idx >= __cfaEditLines.length) {
                calculateCFAInvoiceTotal();
                return;
            }
            var L = __cfaEditLines[__idx++];
            setTimeout(function() {
                if (L.product_id && L.cfa_distributor_stock_id) {
                    addProductToInvoice(L.product_id, L.cfa_distributor_stock_id, L.quantity);
                }
                setTimeout(__loadNextEditLine, 400);
            }, 150);
        }
        setTimeout(__loadNextEditLine, 400);
    }
    ...
    $.easyAjax({
        ...
        success: function(response) {
            if (response.status == 'success' && typeof response.data !== 'undefined') {
                ...
                if (typeof window.cfaEditStockistId === 'number' && window.cfaEditStockistId > 0) {
                    $('#cfa_stockist_id').val(String(window.cfaEditStockistId));
                    $('#cfa_stockist_id').selectpicker('refresh');
                }
            }
        },
        error: function(xhr) {
