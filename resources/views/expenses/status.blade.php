    ...
    <div class="d-flex flex-column w-100 mt-4">
        ...
        <form method="GET" action="{{ route('expenses.status') }}" id="filter-form">
            <div class="row mb-3">
                ...
                <div class="col-md-3 d-flex flex-wrap align-items-end pb-3">
                    <button type="submit" class="btn btn-primary btn-sm mr-2 mb-2">
                        <i class="fa fa-search"></i> Load Expenses
                    </button>
                    <a href="{{ route('expenses.status.approved-export', array_filter(['month' => $selectedMonth, 'employee_id' => $selectedEmployeeId ?? 'all'])) }}"
                       class="btn btn-success btn-sm mr-2 mb-2" title="@lang('modules.expenses.approvedExpensesExportHint')">
                        <i class="fa fa-file-excel"></i> @lang('modules.expenses.downloadApprovedExcel')
                    </a>
                    <x-forms.link-primary :link="route('expenses.create')" class="mb-2 mr-2" icon="plus">
                        Create Expense
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-6">
                            <h5 class="mb-0">
                                ...
                                @php
                                    $statusEmpId = optional($employee->employeeDetail)->employee_id
                                        ?? optional($employee->employeeDetails)->employee_id;
                                @endphp
                                @if($statusEmpId)
                                    <span class="badge badge-light text-dark border ml-1 f-12">{{ $statusEmpId }}</span>
                                @endif
                        ...
                        <table class="table table-bordered table-hover status-table mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th style="min-width: 48px;">Day</th>
                                    <th style="min-width: 110px;">Town Worked</th>
                                    <th style="min-width: 120px;">Worked With</th>
                                    <th style="min-width: 70px;" class="text-center">No. Dr</th>
                                    <th style="min-width: 70px;" class="text-center">No. Retailer</th>
                                    <th style="min-width: 160px;">Head Quarter Visited</th>
                                    <th style="min-width: 90px;">Mode</th>
                                    <th style="min-width: 72px;" class="text-right">Km</th>
                                    <th style="min-width: 72px;" class="text-right">Fare</th>
                                    <th style="min-width: 120px;">Allowances (HQ / Ex / O/S)</th>
                                    <th style="min-width: 72px;" class="text-right">Fixed</th>
                                    <th style="min-width: 72px;" class="text-right">Other</th>
                                    <th style="min-width: 100px;">Remarks</th>
                                    <th style="min-width: 80px;" class="text-right">Total</th>
                                    <th style="min-width: 130px;" class="text-center">Submitted To</th>
                                    <th style="min-width: 100px;" class="text-center">Status</th>
                                    <th style="min-width: 120px;" class="text-center">Approved By</th>
                            ...
                            <tbody>
                                @foreach($groupExpenses as $expense)
                                    ...
                                    @endphp
                                    @php
                                        $workedWithRaw = $expense->worked_with;
                                    ...
                                    <tr class="{{ $statusClass }}" data-status="{{ $expense->status ?? 'pending' }}" data-expense-id="{{ $expense->id }}">
                                        ...
                                        <td><small class="worked-with-cell">{{ $workedWithLabel }}</small></td>
                                        <td class="text-center">{{ (int) ($expense->no_of_doctors_met ?? 0) }}</td>
                                        <td class="text-center">{{ (int) ($expense->no_of_retailers_met ?? 0) }}</td>
                                        <td>
                                            <small class="d-block"><span class="text-muted">From:</span> {{ $expense->headquarter_from ?: '—' }}</small>
                                            <small class="d-block"><span class="text-muted">To:</span> {{ $expense->headquarter_to ?: '—' }}</small>
                                        </td>
                                        <td><small>{{ $expense->mode_of_transport ?: '—' }}</small></td>
                                        <td class="text-right">{{ number_format($expense->km ?? 0, 2) }}</td>
                                        <td class="text-right">₹{{ number_format($expense->fare_rs ?? 0, 2) }}</td>
                                        <td>
                                            <small>
                                        ...
                                        <td class="text-right">₹{{ number_format($expense->fixed_expenses ?? 0, 2) }}</td>
                                        <td class="text-right">₹{{ number_format($expense->other_expenses ?? 0, 2) }}</td>
                                        <td><small class="text-break" title="{{ e($expense->remarks ?? '') }}">{{ $expense->remarks ? \Str::limit($expense->remarks, 80) : '—' }}</small></td>
                                        <td class="text-right"><strong>₹{{ number_format($expense->price, 2) }}</strong></td>
                                ...
                                <tr class="bg-light font-weight-bold">
                                    <td colspan="14" class="text-right"><strong>Group Total:</strong></td>
