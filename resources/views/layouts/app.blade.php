...
<!-- Load moment.js before main.js -->
<script src="{{ asset('vendor/moment/moment-with-locales.min.js') }}"></script>

<!-- Define modal constants BEFORE main.js loads (main.js uses these constants) -->
<script>
    // Define modal constants globally before main.js uses them
...
<script>
    // Make pusher_setting available globally for bootstrap.js
    window.pusher_setting = @json(pusher_settings());
    
    // Translation of default values for the select picker box.
    // Check if selectpicker is available before accessing Constructor
    if (typeof $ !== 'undefined' && $.fn.selectpicker && $.fn.selectpicker.Constructor && $.fn.selectpicker.Constructor.DEFAULTS) {
    ...
    // Keep these constants for backward compatibility (already defined above, but keeping for reference)
    const MODAL_DEFAULT = window.MODAL_DEFAULT;
    const MODAL_LG = window.MODAL_LG;
    const MODAL_XL = window.MODAL_XL;
    const MODAL_HEADING = window.MODAL_HEADING;
    const RIGHT_MODAL = '#task-detail-1';
    const RIGHT_MODAL_CONTENT = '#right-modal-content';
    const RIGHT_MODAL_TITLE = '#right-modal-title';
    const company = @json(companyOrGlobalSetting());
    const pusher_setting = window.pusher_setting;
    const message_setting = @json(message_setting());
    window.SEARCH_KEYWORD = "{{ request('search_keyword') }}";
    const SEARCH_KEYWORD = window.SEARCH_KEYWORD;
    ...
    // Make daterangeLocale and daterangeConfig available globally
    window.daterangeConfig = {
    ...
    // daterangeConfig and daterangeLocale are now defined above as window properties
    const daterangeConfig = window.daterangeConfig;
    const daterangeLocale = window.daterangeLocale;
...
<script>
    // Initialize Echo/Pusher if settings are available
    if (typeof window.pusher_setting !== 'undefined' && window.pusher_setting && window.pusher_setting.status && window.pusher_setting.pusher_app_key) {
        if ((window.Echo === null || typeof window.Echo === 'undefined') && window.EchoConstructor && window.Pusher) {
            // EchoConstructor was stored in bootstrap.js, now initialize it properly
            try {
                window.Echo = new window.EchoConstructor({
                    broadcaster: 'pusher',
                    key: window.pusher_setting.pusher_app_key,
                    cluster: window.pusher_setting.pusher_cluster || 'mt1',
                    forceTLS: window.pusher_setting.force_tls || true
                });
            } catch (e) {
    ...
    // Ensure init function is available globally
    if (typeof init === 'undefined') {
        console.warn('init function is not defined. Make sure custom.js is loaded.');
    } else {
