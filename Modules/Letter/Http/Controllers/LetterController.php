...
class LetterController extends AccountBaseController
{
    ...
    public function __construct()
    {
        ...
        // Module check removed - Letter module is always activated
    ...
    private function userForLetterVariables(?int $userId): ?User
    {
        if (! $userId) {
            return null;
        }
    ...
    private function employeeLetterVariable($letter)
    {
        ...
        if ($letter->user_id) {
            $user = $this->userForLetterVariables($letter->user_id) ?: $letter->user;
            $letterVariable = LetterVariable::getMappedValues($user);
        }
        else {
            $letterVariable = [
