...
class SalaryMonthlyReport implements FromCollection, WithHeadings, WithColumnFormatting, WithEvents, ShouldAutoSize
{
    ...
    public function collection()
    {
        ...
        $query = SalarySlip::select([
            ...
            ->leftJoin('pharma_headquarters', 'pharma_headquarters.id', '=', 'employee_details.headquarter_id');
        ...
        // Fixed headings before dynamic Headings (common report fields: Emp code, Name, DOJ, Department, Designation, HQ)
        $columns = [
            __('app.employeeId'),
            __('app.employee'),
        ...
        $results->each(function ($result, $index) use (&$rows, $headsArray, &$processedData, &$previousEmployeeId, &$finalTotals, $recordCount) {
            ...
            $dojFormatted = $isFirstEntryForEmployee && $result->joining_date
                ? Carbon::parse($result->joining_date)->format(company()->date_format)
                : ($isFirstEntryForEmployee ? '-' : '');
            $hqName = $isFirstEntryForEmployee ? ($result->headquarter_name ?? '-') : '';

            // Define initial row structure (common fields then Salary Group, Month, Basic Salary)
            $row = [
            ...
            $fixedAllowance = ($result->fixed_allowance > 0) ? $result->fixed_allowance : 0;
