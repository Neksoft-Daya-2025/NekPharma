...
class PayrollController extends AccountBaseController
{
    ...
    /**
     * Whether attendance is finalised for the calendar month of the payroll period start (same rule as generatePaySlip).
     */
    public function attendanceFinalisationStatus(Request $request)
    {
        $viewPermission = user()->permission('view_payroll');
        ...
        if (trim((string) $monthRaw) === '') {
            return Reply::error(__('messages.invalidRequest'));
        }

        $parts = explode(' ', trim((string) $monthRaw));
        if (empty($parts[0])) {
            return Reply::error(__('messages.invalidRequest'));
        }
        ...
        $monthNum = (int) $startDate->month;
        $monthPadded = str_pad((string) $monthNum, 2, '0', STR_PAD_LEFT);
        $finalised = AttendanceMonthFinalisation::isFinalised(company()->id, $year, $monthNum);

        $attendanceUrl = route('attendances.index').'?'.http_build_query([
            'year' => (string) $year,
            'month' => $monthPadded,
        ]);
    ...
    public function generatePaySlip(Request $request)
    {
        ...
        // SRS 3.1.3: Payroll can be generated only after attendance for the month is finalised
        if (!AttendanceMonthFinalisation::isFinalised(company()->id, $startDate->year, $startDate->month)) {
            return Reply::error(__('messages.attendanceMonthNotFinalised'));
        }

        // Process deductions file if uploaded
        $uploadedDeductions = [];
        if ($request->hasFile('deductions_file')) {
            $uploadedDeductions = $this->processDeductionsFile($request->file('deductions_file'));
        }
        ...
        foreach ($users as $user) {
            ...
            if ($endDate->greaterThan($joiningDate)) {
                ...
                if($useAttendance){
                    ...
                    if($unPaidAmount > 0){
                        $deductions[__('payroll::modules.payroll.unpaidDaysDeduction', ['days' => (int) $unpaidDayCount])] = $unPaidAmount;
                        $unpaidDaysAmount = $unPaidAmount;
                    }
                }

                // Add deductions from uploaded Excel file
                if (isset($uploadedDeductions[$userId]) && is_array($uploadedDeductions[$userId])) {
    ...
    public function downloadDeductionSample()
    {
        $this->addPermission = user()->permission('add_payroll');
        abort_403(!in_array($this->addPermission, ['all', 'added']));
        ...
        $headers = ['Employee ID', 'Employee Email', 'Employee Name', 'Department', 'Deduction Name', 'Deduction Amount'];
        ...
        $instructions = [
            ['Salary Deductions Import Template - Instructions'],
            [''],
            ['Column Descriptions:'],
            ['Employee ID', 'The unique employee ID (e.g., RVB / 101, RVB / 102) - REQUIRED if Email not provided'],
            ['Employee Email', 'Employee email address registered in the system - REQUIRED if ID not provided'],
        ...
        $fileName = 'salary_deductions_template_' . now()->format('Y-m-d') . '.xlsx';
    ...
    private function processDeductionsFile($file)
    {
        try {
            ...
            foreach ($rows as $rowIndex => $row) {
                // Skip if both Employee ID and Email are empty
                ...
                // Column 1: Employee Email
                // Column 2: Employee Name (for reference, not used for matching)
                // Column 3: Department (for reference, not used for matching)
                ...
                if ($deductionAmount <= 0) {
                    $skippedCount++;
                    \Log::warning("Deductions import: Row " . ($rowIndex + 2) . " skipped - Invalid amount: {$deductionAmount}");
                    continue;
                }
                ...
                if (!$employee && !empty($employeeEmail)) {
                    $employee = User::where('email', $employeeEmail)
                        ->select('id', 'name', 'email')
                        ->first();
                }

                if ($employee) {
                    if (!isset($deductions[$employee->id])) {
                    ...
                    $processedCount++;
                    \Log::info("Deductions import: Added {$deductionName} ({$deductionAmount}) for {$employee->name}");
                } else {
                    $skippedCount++;
                    \Log::warning("Deductions import: Row " . ($rowIndex + 2) . " skipped - Employee not found: ID={$employeeId}, Email={$employeeEmail}");
                }
            }

            \Log::info("Deductions import completed: {$processedCount} deductions processed, {$skippedCount} rows skipped");
            return $deductions;
