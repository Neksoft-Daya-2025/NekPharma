<div class="row">
    ...
    @foreach ($salaryComponents->where('component_type', 'earning') as $salaryComponent)
        @php
            $componentValue = $payrollController->componentMonthlyValue($salaryComponent, $annualSalary, $basicSalary, $employeeVariableSalaries);
        @endphp
        <div class="col-md-12 mt-1">
            <div class="row">
                ...
                <div class="col-md-3">
                    <x-forms.input-group>
                        <x-slot name="prepend" id="currency"><span class="input-group-text f-14 bg-white-shade">{{ ($currency->currency ? $currency->currency->currency_symbol : company()->currency->currency_symbol ) }}</span></x-slot>
                        <input type="number" step="0.01" min="0" class="form-control height-35 f-14 manual-component earning-component" name="earning_variable[{{ $salaryComponent->id }}]" data-type="earning" data-type-id="{{ $salaryComponent->id }}" id="variable-{{ $salaryComponent->id }}" value="{{ round($componentValue, 2) }}">
                    </x-forms.input-group>
                </div>
                <div class="col-md-3">
                    <x-forms.input-group>
                        <x-slot name="prepend" id="currency"><span class="input-group-text f-14 bg-white-shade">{{ ($currency->currency ? $currency->currency->currency_symbol : company()->currency->currency_symbol ) }}</span></x-slot>
                        <input type="text" class="form-control height-35 f-14 component-annual" id="variableAnually{{ $salaryComponent->id }}" value="{{ $payrollController->currencyFormatterCustom($componentValue * 12) }}" readonly>
                    </x-forms.input-group>
                </div>
            </div>
        </div>
            ...
            <div class="col-md-12 mt-1">
                <div class="row">
                    ...
                    <div class="col-md-3">
                        <x-forms.input-group>
                            <x-slot name="prepend" id="currency"><span class="input-group-text f-14 bg-white-shade">{{ ($currency->currency ? $currency->currency->currency_symbol : company()->currency->currency_symbol ) }}</span></x-slot>
                            <input type="number" step="0.01" min="0" class="form-control height-35 f-14 manual-component deduction-component" data-type="deduction" data-type-id="{{ $salaryComponent->id }}" data-component-name="{{ strtolower($salaryComponent->component_name) }}" name="deduction_variable[{{ $salaryComponent->id }}]" id="deductionVariable{{ $salaryComponent->id }}" value="{{ round($componentValue, 2) }}">
                        </x-forms.input-group>
                    </div>
                    <div class="col-md-3">
                        <x-forms.input-group>
                            <x-slot name="prepend" id="currency"><span class="input-group-text f-14 bg-white-shade">{{ ($currency->currency ? $currency->currency->currency_symbol : company()->currency->currency_symbol ) }}</span></x-slot>
                            <input type="text" class="form-control height-35 f-14 component-annual" id="variableAnuallyDeduction{{ $salaryComponent->id }}" value="{{ $payrollController->currencyFormatterCustom($componentValue * 12) }}" readonly>
                        </x-forms.input-group>
                    </div>
                </div>
            </div>
...
<script>
    $('#annual_salary').val({{ $annualSalary }}).attr('data-component-calculated-ctc', 'true');
    ...
    function getBasicCalculations() {
        ...
        $.easyAjax({
            ...
            disableButton: true,
            blockUI: true,
            data: { basicType: basicType, basicValue: basicValue, annualSalary: annualSalary, userId: userId },
            success: function (response) { $('#components').html(response.component); }
        });
    }

    function changeClc() {
        var basicSalary = $('#basic_value').val();
        if (basicSalary > 0) { getBasicCalculations(); }
    }
</script>
