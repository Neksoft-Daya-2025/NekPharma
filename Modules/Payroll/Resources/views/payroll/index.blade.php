    ...
    <div class="content-wrapper">
        ...
        @if($addPayrollPermission == 'all' || $addPayrollPermission == 'added')
            <div class="card bg-white border-0 b-shadow-4">
                ...
                <div class="card-body">
                    <div id="payroll-attendance-finalisation-banner" class="alert mb-3 d-none" role="alert">
                        <i class="fa fa-calendar-check-o mr-1" id="payroll-attendance-finalisation-icon" aria-hidden="true"></i>
                        <span id="payroll-attendance-finalisation-text"></span>
                        <a id="payroll-attendance-finalisation-link" href="#" class="font-weight-bold ml-2 d-none">@lang('payroll::modules.payroll.goToAttendance')</a>
                    </div>
                    <x-form id="genrate-payroll-form" method="PUT">
                        <div class="row">
                            ...
                            <div class="col-12 mb-3">
                                <hr class="my-3">
                                <h5 class="f-16 text-dark"><i class="fa fa-file-excel text-success"></i> Upload Deductions (Optional)</h5>
                                <p class="text-muted f-12 mb-2">Upload an Excel file with employee deductions. The file should have columns: <strong>Employee ID</strong>, <strong>Employee Email</strong>, <strong>Employee Name</strong>, <strong>Department</strong>, <strong>Deduction Name</strong>, <strong>Deduction Amount</strong></p>
                            </div>

                            <div class="col-md-6 mb-4">
                                <x-forms.file 
                                    fieldLabel="Deductions Excel File" 
                                    fieldName="deductions_file" 
                                    fieldId="deductions_file"
                                    allowedFileExtensions="xlsx xls csv"
                                    :popover="'Upload Excel file with deduction details. Format: Employee ID, Employee Email, Employee Name, Department, Deduction Name, Deduction Amount'">
                                </x-forms.file>
                                <small class="form-text text-muted">
                                    <a href="javascript:void(0);" id="download-deduction-sample" class="text-primary">
                                        <i class="fa fa-download"></i> Download Sample Excel Template
                                    </a>
                                </small>
                            </div>
...
@push('scripts')
    ...
    <script>
        var payrollAttendanceMsg = {
            finalised: @json(__('payroll::modules.payroll.attendanceFinalisedForPayrollPeriod')),
            notFinalised: @json(__('payroll::modules.payroll.attendanceNotFinalisedForPayrollPeriod')),
            selectPeriod: @json(__('payroll::modules.payroll.selectPayrollPeriodForAttendanceStatus'))
        };

        function refreshPayrollAttendanceFinalisationBanner() {
            var monthVal = $('#month').val();
            var $banner = $('#payroll-attendance-finalisation-banner');
            var $link = $('#payroll-attendance-finalisation-link');
            var $icon = $('#payroll-attendance-finalisation-icon');
            if (!$banner.length) {
                return;
            }
            if (!monthVal || monthVal === 'all') {
                $banner.removeClass('alert-success alert-warning').addClass('alert-info');
                $icon.removeClass('fa-check-circle fa-exclamation-triangle').addClass('fa-info-circle');
                $('#payroll-attendance-finalisation-text').text(payrollAttendanceMsg.selectPeriod);
                $link.addClass('d-none');
                $banner.removeClass('d-none');
                return;
            }
            $.easyAjax({
                url: "{{ route('payroll.attendance-finalisation-status') }}",
                type: "GET",
                data: { month: monthVal },
                success: function (response) {
                    ...
                    $banner.removeClass('d-none alert-success alert-warning alert-info');
                    $link.addClass('d-none');
                    if (response.finalised) {
                        $banner.addClass('alert-success');
                        $icon.removeClass('fa-info-circle fa-exclamation-triangle').addClass('fa-check-circle');
                        $('#payroll-attendance-finalisation-text').text(
                            payrollAttendanceMsg.finalised.replace(':period', response.label)
                        );
                    } else {
                        $banner.addClass('alert-warning');
                        $icon.removeClass('fa-info-circle fa-check-circle').addClass('fa-exclamation-triangle');
                        $('#payroll-attendance-finalisation-text').text(
                            payrollAttendanceMsg.notFinalised.replace(':period', response.label)
                        );
                        $link.attr('href', response.attendance_url).removeClass('d-none');
                    }
                }
            });
        }
        ...
        $('body').on('click', '#generate-payslip', function () {
            ...
            if (employee_id && employee_id.length > 0) {
                employee_id.forEach(function(id) {
                    formData.append('employee_id[]', id);
                });
            }
            
            // Add deductions file if uploaded
        ...
        // Download Deduction Sample Template
        $('#download-deduction-sample').on('click', function() {
            var link = document.createElement('a');
            link.href = '{{ route("payroll.download-deduction-sample") }}';
            link.download = 'salary_deductions_template.xlsx';
            link.click();
        });
