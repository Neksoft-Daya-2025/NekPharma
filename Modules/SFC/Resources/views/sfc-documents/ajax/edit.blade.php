<x-form id="update-sfc-document-form">
    ...
    <div class="row p-20">
        ...
        <div class="col-md-3">
            <x-forms.text :fieldLabel="__('sfc::app.name')" fieldName="name" fieldId="name" :fieldValue="$document->name ?? ($employeeName ?? user()->name)" fieldPlaceholder="Employee Name"/>
        </div>
        <div class="col-md-3">
            <x-forms.text :fieldLabel="__('sfc::app.headquarter')" fieldName="headquarter" fieldId="headquarter" :fieldValue="$document->headquarter ?? ($employeeHeadquarter ?? '')" fieldPlaceholder="Headquarter"/>
        </div>
        <div class="col-md-3">
            <x-forms.text :fieldLabel="__('app.area')" fieldName="area" fieldId="area" :fieldValue="$document->area ?? ($employeeArea ?? '')" fieldPlaceholder="Area"/>
        </div>
        <div class="col-md-3">
            <x-forms.text :fieldLabel="__('app.region')" fieldName="region" fieldId="region" :fieldValue="$document->region ?? ($employeeRegion ?? '')" fieldPlaceholder="Region"/>
        </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <input type="number" name="vip_dr_count" id="vip_dr_count" class="form-control text-center" value="{{ $document->vip_dr_count }}" placeholder="52" required min="0"/>
                            </td>
                            <td class="text-center">
                                <input type="number" name="core_dr_count" id="core_dr_count" class="form-control text-center" value="{{ $document->core_dr_count }}" placeholder="48" required min="0"/>
                            </td>
                            <td class="text-center">
                                <input type="number" name="total_dr_count" id="total_dr_count_summary" class="form-control text-center" value="{{ $document->total_dr_count }}"/>
                            </td>
                        </tr>
                        <tr>
                            <td><strong>@lang('sfc::app.noOfVisitInMonthEachDr')</strong></td>
                            <td class="text-center">
                                <input type="number" name="vip_visits_per_month" id="vip_visits_per_month" class="form-control text-center" value="{{ $document->vip_visits_per_month }}" placeholder="2" required min="0"/>
                            </td>
                            <td class="text-center">
                                <input type="number" name="core_visits_per_month" id="core_visits_per_month" class="form-control text-center" value="{{ $document->core_visits_per_month }}" placeholder="4" required min="0"/>
                            </td>
                            <td class="text-center">
                                <input type="number" name="total_visits_per_month" id="total_visits_per_month_summary" class="form-control text-center" value="{{ $document->vip_visits_per_month + $document->core_visits_per_month }}"/>
                            </td>
                        </tr>
                        <tr>
                            <td><strong>@lang('sfc::app.totalVisitInMonth')</strong></td>
                            <td class="text-center">
                                <input type="number" name="total_vip_visits_monthly" id="total_vip_visits_monthly" class="form-control text-center" value="{{ $document->total_vip_visits_monthly }}"/>
                            </td>
                            <td class="text-center">
                                <input type="number" name="total_core_visits_monthly" id="total_core_visits_monthly" class="form-control text-center" value="{{ $document->total_core_visits_monthly }}"/>
                            </td>
                            <td class="text-center">
                                <input type="number" name="total_visits_monthly" id="total_visits_monthly_summary" class="form-control text-center" value="{{ $document->total_visits_monthly }}"/>
                            </td>
                        </tr>
                    </tbody>
                </table>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-sm" id="sfc-items-table" style="background-color: #e6ffe6;">
                    ...
                    <tbody id="sfc-items-tbody">
                        @foreach($document->chartItems as $index => $item)
                        <tr data-row-index="{{ $index + 1 }}">
                            <td>{{ $index + 1 }}</td>
                            <td><input type="text" name="items[{{ $index + 1 }}][covered_from]" class="form-control form-control-sm" value="{{ $item->covered_from }}"/></td>
                            <td>
                                <select name="items[{{ $index + 1 }}][town_name]" class="form-control form-control-sm town-select">
                                    <option value="">-- Select Town --</option>
                                    @foreach($towns ?? [] as $town)
                                        <option value="{{ $town['name'] }}" @selected($item->town_name == $town['name'])>{{ $town['name'] }} ({{ $town['type'] }})</option>
                                    @endforeach
                                </select>
                            </td>
                            <td>
                                @php
                                    $actualKmValue = is_array($item->one_way_km_actual) ? (count($item->one_way_km_actual) > 0 ? array_sum($item->one_way_km_actual) : null) : $item->one_way_km_actual;
                                @endphp
                                <input type="number" step="0.01" name="items[{{ $index + 1 }}][one_way_km_actual]" class="form-control form-control-sm" value="{{ $actualKmValue }}"/>
                            </td>
                            <td><input type="number" step="0.01" name="items[{{ $index + 1 }}][grace]" class="form-control form-control-sm" value="{{ $item->grace }}"/></td>
                            <td><input type="number" step="0.01" name="items[{{ $index + 1 }}][total_km]" class="form-control form-control-sm" value="{{ $item->total_km }}"/></td>
                            <td><input type="number" step="0.01" name="items[{{ $index + 1 }}][two_way_fare]" class="form-control form-control-sm" value="{{ $item->two_way_fare }}"/></td>
                            <td><input type="number" step="0.01" name="items[{{ $index + 1 }}][one_way_fare]" class="form-control form-control-sm" value="{{ $item->one_way_fare }}"/></td>
                            <td>
                                <select name="items[{{ $index + 1 }}][ex_hq_os]" class="form-control form-control-sm">
                                    <option value="">--</option>
                                    <option value="HQ" @selected($item->ex_hq_os == 'HQ')>HQ</option>
                                    <option value="EX" @selected($item->ex_hq_os == 'EX')>EX</option>
                                    <option value="OS" @selected($item->ex_hq_os == 'OS')>OS</option>
                                </select>
                            </td>
                            <td>
                                <select name="items[{{ $index + 1 }}][mode_of_travel]" class="form-control form-control-sm mode-of-travel-select">
                                    <option value="">--</option>
                                    <option value="Bus" @selected($item->mode_of_travel == 'Bus')>@lang('sfc::app.bus')</option>
                                    <option value="Train" @selected($item->mode_of_travel == 'Train')>@lang('sfc::app.train')</option>
                                    <option value="Car" @selected($item->mode_of_travel == 'Car')>@lang('sfc::app.car')</option>
                                    <option value="Bike" @selected($item->mode_of_travel == 'Bike')>Bike</option>
                                    <option value="Flight" @selected($item->mode_of_travel == 'Flight')>@lang('sfc::app.flight')</option>
                                    <option value="Other" @selected($item->mode_of_travel == 'Other')>@lang('sfc::app.other')</option>
                                </select>
                            </td>
                            <td>
                                <div class="bootstrap-timepicker">
                                    <input type="text" name="items[{{ $index + 1 }}][time_in_hours]" class="form-control form-control-sm time-picker-input" placeholder="HH:MM AM/PM" value="{{ $item->time_in_hours }}"/>
                                </div>
                            </td>
                            <td><input type="number" name="items[{{ $index + 1 }}][no_of_days_monthly]" class="form-control form-control-sm" value="{{ $item->no_of_days_monthly }}"/></td>
                            <td><input type="number" name="items[{{ $index + 1 }}][vip_dr_count]" class="form-control form-control-sm" value="{{ $item->vip_dr_count }}"/></td>
                            <td><input type="number" name="items[{{ $index + 1 }}][core_dr_count]" class="form-control form-control-sm" value="{{ $item->core_dr_count }}"/></td>
                            <td><input type="text" name="items[{{ $index + 1 }}][total_dr_count]" class="form-control form-control-sm" value="{{ $item->total_dr_count }}"/></td>
                            <td>
                                <select name="items[{{ $index + 1 }}][stockist_name]" class="form-control form-control-sm stockist-select">
                                    <option value="">-- Select Stockist --</option>
                                    @foreach($stockists ?? [] as $stockist)
                                        <option value="{{ $stockist->shopname }}" @selected($item->stockist_name == $stockist->shopname)>{{ $stockist->shopname }}{{ $stockist->fullname ? ' (' . $stockist->fullname . ')' : '' }}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td><input type="number" step="0.01" name="items[{{ $index + 1 }}][current_business]" class="form-control form-control-sm" value="{{ $item->current_business }}"/></td>
                            <td><input type="number" step="0.01" name="items[{{ $index + 1 }}][approx_business_expected]" class="form-control form-control-sm" value="{{ $item->approx_business_expected }}"/></td>
                            <td><input type="text" name="items[{{ $index + 1 }}][remarks]" class="form-control form-control-sm" value="{{ $item->remarks }}"/></td>
                            <td>
        ...
        <div class="col-md-6 mt-3">
            <x-forms.text :fieldLabel="__('sfc::app.filledByName')" fieldName="filled_by_name" fieldId="filled_by_name" :fieldValue="$document->filled_by_name" fieldPlaceholder="Enter NAME"/>
        </div>

        <div class="col-md-6 mt-3">
            <x-forms.text :fieldLabel="__('sfc::app.abmApproval')" fieldName="abm_approval" fieldId="abm_approval" :fieldValue="$document->abm_approval" fieldPlaceholder="ABM Approval" :fieldReadOnly="true"/>
        </div>

        <div class="col-md-6 mt-3">
            <x-forms.text :fieldLabel="__('sfc::app.rbmApproval')" fieldName="rbm_approval" fieldId="rbm_approval" :fieldValue="$document->rbm_approval" fieldPlaceholder="RBM Approval" :fieldReadOnly="true"/>
        </div>
    </div>

    <x-form-actions>
        <x-forms.button-primary id="update-sfc-document-btn" class="mr-3" icon="check">@lang('app.update')</x-forms.button-primary>
...
$(document).ready(function() {
    let rowCount = {{ $document->chartItems->count() > 0 ? $document->chartItems->count() : 0 }};
    ...
    // Calculate totals on page load for edit page
            ...
            <tr data-row-index="${rowCount}">
                ...
                <td>
                    <select name="items[${rowCount}][town_name]" class="form-control form-control-sm town-select">
                        <option value="">-- Select Town --</option>
                        @foreach($towns ?? [] as $town)
                            <option value="{{ $town['name'] }}">{{ $town['name'] }} ({{ $town['type'] }})</option>
                        @endforeach
                    </select>
                </td>
    ...
    // Form submission
    $('#update-sfc-document-btn').click(function(e) {
        e.preventDefault();
        // Calculate totals before submission
        calculateTotals();
        
        const url = "{{ route('sfc-charts.update', $document->id) }}";
        $.easyAjax({
            url: url,
            container: '#update-sfc-document-form',
            type: "PUT",
            disableButton: true,
            blockUI: true,
            buttonSelector: "#update-sfc-document-btn",
            data: $('#update-sfc-document-form').serialize(),
