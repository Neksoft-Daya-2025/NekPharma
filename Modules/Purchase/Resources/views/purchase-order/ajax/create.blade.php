...
<script src="{{ asset('vendor/jquery/dropzone.min.js') }}"></script>
    ...
    $(document).ready(function() {
                ...
                $.easyAjax({
                    ...
                    success: function(response) {
                        if (response.status == 'success' && response.view) {
                            ...
                            // Initialize select picker and datepicker for new row
        ...
        $(document).on('change', '.purchase_line_tax_id', function() {
            ...
            if (row) {
                // Update hidden field
                $(this).closest('tr').find('input[name*="[purchase_line_tax_id]"]').val($(this).val());
                
                // Recalculate purchase_price_inc_tax based on current purchase price and tax
                var purchasePrice = parseFloat($('.purchase_unit_cost[data-row="' + row + '"]').val()) || 0;
                var taxRate = 0;
                if ($(this).val()) {
                    taxRate = parseFloat($(this).find('option:selected').data('rate')) || 0;
                }
                var purchasePriceIncTax = purchasePrice * (1 + (taxRate / 100));
                $('.purchase_unit_cost_after_tax[data-row="' + row + '"]').val(purchasePriceIncTax.toFixed(2));
                $('.purchase_unit_cost_after_tax[data-row="' + row + '"]').closest('tr').find('input[name*="[purchase_price_inc_tax]"]').val(purchasePriceIncTax.toFixed(2));
        ...
        function calculateEnhancedTotal() {
            ...
            // Calculate tax from all rows
            $('.purchase_product_unit_tax').each(function() {
                var taxAmount = parseFloat($(this).val()) || 0;
                var row = $(this).data('row');
                var taxName = $('.purchase_line_tax_id[data-row="' + row + '"] option:selected').text();
                
                if (taxName && taxAmount > 0) {
