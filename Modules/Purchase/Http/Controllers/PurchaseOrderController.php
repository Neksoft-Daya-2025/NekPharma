    ...
    public function store(StoreRequest $request)
    {
        ...
        if (!empty($purchases)) {
            ...
            foreach ($purchases as $index => $purchase) {
                ...
                $purchaseItem->purchase_line_tax_id = $purchase['purchase_line_tax_id'] ?? ($purchaseLineTaxIds[$index] ?? null);
                $purchaseItem->save();

                // Attach taxes - support both old format (taxes[index][]) and new format
                $itemTaxIds = [];
            ...
            foreach ($itemNames as $index => $itemName) {
                ...
                $purchaseItem->purchase_line_tax_id = $purchaseLineTaxIds[$index] ?? null;
                $purchaseItem->save();

                // Attach taxes
                if (isset($taxes[$index]) && is_array($taxes[$index]) && !empty($taxes[$index])) {
                    $purchaseItem->taxes()->sync($taxes[$index]);
                } elseif (isset($purchaseLineTaxIds[$index]) && !empty($purchaseLineTaxIds[$index])) {
                    $purchaseItem->taxes()->sync([$purchaseLineTaxIds[$index]]);
                }
            }
        }
