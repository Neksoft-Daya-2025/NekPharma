...
class PurchaseProductsDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->editColumn('stock', function ($row) {
            // Calculate current stock: opening_stock + all purchase entries
            // This is stored in PurchaseStockAdjustment.net_quantity
            $currentStock = 0;
            
            // Get the net_quantity from inventory (which includes opening_stock + purchase entries)
            foreach ($row->inventory as $inventory) {
                $currentStock = $inventory->net_quantity ?: 0;
                break; // Get the first inventory record
            }
            
            // If no inventory record exists, calculate from opening_stock + purchase entries
            if ($currentStock == 0) {
                $openingStock = $row->opening_stock ?? 0;
                $totalPurchaseEntries = \App\Models\ProductPurchaseDetail::where('product_id', $row->id)
                    ->sum('quantity');
    ...
    public function query(PurchaseProduct $model)
    {
        ...
        $model = $model->with('tax', 'category', 'subCategory', 'inventory','unit')
            ...
            ->select('products.id', 'products.name', 'products.packing', 'products.total', 'products.price', 'products.status', 'products.taxes', 'products.unit_id', 'products.opening_stock', 'products.allow_purchase', 'products.added_by', 'products.default_image', 'products.category_id', 'products.sub_category_id', 'products.description');
    ...
    protected function getColumns()
    {
        ...
        $data = [
            ...
            __('MRP') => ['data' => 'total', 'name' => 'total', 'title' => __('MRP')],
