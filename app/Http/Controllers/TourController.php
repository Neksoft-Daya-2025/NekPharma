...
class TourController extends AccountBaseController
{
    ...
    public function create(Request $request)
    {
        ...
        if (user()->hasAdminLikeAccess() && $forEmployeeId) {
        ...
        if (config('app.debug')) {
            \Log::info('TourController::create - Debug Info', [
                'user_id' => user()->id,
                'user_name' => user()->name,
                'is_admin' => user()->hasAdminLikeAccess(),
    ...
    public function store(Request $request)
    {
        ...
        $targetUserId = user()->id;
        $targetUser = null;
        if (user()->hasAdminLikeAccess() && $request->filled('for_employee_id')) {
        ...
        if (TourMonthLock::isLocked(company()->id, (int) $month->year, (int) $month->month) && !user()->hasAdminLikeAccess()) {
    ...
    public function edit($id)
    {
        ...
        if ($this->tour->approved && !user()->hasAdminLikeAccess()) {
        ...
        if (TourMonthLock::isLocked(company()->id, (int) $tourDate->year, (int) $tourDate->month) && !user()->hasAdminLikeAccess()) {
    ...
    public function update(Request $request, $id)
    {
        ...
        // After approval: tour plan becomes locked (only admin can edit)
        if ($tour->approved && !user()->hasAdminLikeAccess()) {
