...
class LedgerController extends AccountBaseController
{
    ...
    public function dataCFAStockistLedger(Request $request)
    {
        ...
        $partyId = (int) $partyId;
        if (in_array('client', user_roles(), true) && !PharmaDesignationHelper::hasFullCFAAccess()) {
            $stockistOk = CFAStockist::where('company_id', company()->id)
                ->where('id', $partyId)
    ...
    {
        if (in_array('client', user_roles(), true) && $clientId !== (int) user()->id) {
